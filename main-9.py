...
HOUSE_EDGE = float(os.getenv("HOUSE_EDGE") or "0.07")  # RTP ≈ 93%
MIN_BET = 50
SHOP_MARKUP = 1.35
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    if tv <= 0 or iv <= 0:
        return 0.01
    raw = (iv / tv) * 100.0
    # x2 ≈ 47% (raw=50 * 0.94) — RTP высокий, касса чуть в плюсе
    ch = max(0.01, min(80.0, raw * 0.94))
    ch = min(94.0, ch + float(bonus or 0))
    return round(ch, 2)

def roll_crash_point() -> float:
    r = random.random()
    # ~5% краш 1.00x, фактор 0.94 → RTP ~93–94%
    if r < 0.05:
        return 1.0
    p = max(1.01, 0.94 / (1 - r))
...
def mines_mult(mines: int, opened: int) -> float:
    ...
    for i in range(opened):
        ...
        if safe <= 0:
            break
        mult *= remaining / safe
    return max(1.0, mult * (1 - max(HOUSE_EDGE, 0.07)))  # мины: RTP ~93%
