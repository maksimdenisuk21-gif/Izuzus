...
HOUSE_EDGE = float(os.getenv("HOUSE_EDGE") or "0.10")
MIN_BET = 50
SHOP_MARKUP = 1.35
...
CASES = {
    ...
    # RTP ~96%: mult [0.48,0.68,0.88,1.0,1.2,1.8,3.0,5.0] × w [26,26,18,12,8,5,3,2]
    'star_case_1': {'name': '⭐ STAR CASE I', 'price': 55, 'category': 'stars', 'icon': '⭐', 'color': 'c-starter', 'star_drops': [26, 37, 48, 55, 66, 99, 165, 275], 'star_weights': [26, 26, 18, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 26–275⭐'},
    'star_case_2': {'name': '⭐ STAR CASE II', 'price': 110, 'category': 'stars', 'icon': '✨', 'color': 'c-starter', 'star_drops': [53, 75, 97, 110, 132, 198, 330, 550], 'star_weights': [26, 26, 18, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 53–550⭐'},
    'star_case_3': {'name': '⭐ STAR CASE III', 'price': 280, 'category': 'stars', 'icon': '🌟', 'color': 'c-pepe', 'star_drops': [134, 190, 246, 280, 336, 504, 840, 1400], 'star_weights': [26, 26, 18, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 134–1400⭐'},
    'star_case_4': {'name': '⭐ STAR CASE IV', 'price': 550, 'category': 'stars', 'icon': '💫', 'color': 'c-tg', 'star_drops': [264, 374, 484, 550, 660, 990, 1650, 2750], 'star_weights': [26, 26, 18, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 264–2750⭐'},
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    if tv <= 0 or iv <= 0:
        return 0.01
    raw = (iv / tv) * 100.0
    # x2 ≈ 48% (raw=50 * 0.96) — RTP ~96%
    ch = max(0.01, min(78.0, raw * 0.96))
    ch = min(92.0, ch + float(bonus or 0))
    return round(ch, 2)

def roll_crash_point() -> float:
    r = random.random()
    # ~4% instant 1.00x, factor 0.96 → RTP ~96%
    if r < 0.04:
        return 1.0
    p = max(1.01, 0.96 / (1 - r))
...
def mines_mult(mines: int, opened: int) -> float:
    ...
    for i in range(opened):
        ...
        if safe <= 0:
            break
        mult *= remaining / safe
    return max(1.0, mult * (1 - max(HOUSE_EDGE, 0.10)))  # мины: дом ~10%
...
def _pick_gift_for_case(pool: list, price: int) -> dict:
    """Гифт с умеренным house edge (касса в плюсе, но не жесть)."""
    ...
    # мягкий дом: режем только явный жир
    if val > price * 2.5:
...
def roll_case_drop(case_id: str, c: dict) -> dict:
    """Return {kind: gift|stars, ...}. EV чуть ниже цены, без нищеты."""
...
async def battle_list(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    rooms = []
    for rid, R in list(BATTLES.items()):
        if R.get("done"):
            continue
        ...
        rooms.append({
            "id": rid,
            "case_id": R["case_id"],
            "case_name": c.get("name") or R["case_id"],
            "price": int(c.get("price") or 0),
            "host_name": R.get("host_name") or "Игрок",
...
async def battle_create(body: BattleCreateRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    for R in BATTLES.values():
        if not R.get("done") and R.get("host_id") == u["tg_id"]:
    ...
    BATTLES[rid] = {
        ...
        "host_id": u["tg_id"],
        "host_name": u.get("username") or str(u["tg_id"]),
        "price": price,
        "done": False,
    }
...
async def battle_cancel(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    for rid, R in list(BATTLES.items()):
        if not R.get("done") and R.get("host_id") == u["tg_id"]:
            refund = int(R.get("price") or 0)
            u["balance"] += refund
            await save_user(u)
            BATTLES.pop(rid, None)
            return {"ok": True, "balance": u["balance"]}
...
async def battle_join(body: BattleJoinRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    R = BATTLES.get(body.room_id)
    if not R or R.get("done"):
        raise HTTPException(400, "Комната недоступна")
    if R["host_id"] == u["tg_id"]:
        raise HTTPException(400, "Нельзя войти в свою")
    c = CASES.get(R["case_id"])
    if not c:
        raise HTTPException(400, "Кейс не найден")
    price = int(R.get("price") or c.get("price") or 0)
    if u["balance"] < price:
        raise HTTPException(400, "Недостаточно ⭐")
    u["balance"] -= price
    await save_user(u)
    # roll both
    drop_h = roll_case_drop(R["case_id"], c)
    drop_j = roll_case_drop(R["case_id"], c)
    def drop_val(d):
        if d.get("kind") == "stars":
    ...
    def drop_view(d):
        if d.get("kind") == "stars":
            ...
            return {"name": f"⭐ {s}", "value": s, "emoji": "⭐"}
        g = d.get("gift") or {}
        return {"name": g.get("name") or "?", "value": int(g.get("value") or 0), "emoji": g.get("emoji") or "🎁", "img": g.get("img") or ""}
    vh, vj = drop_val(drop_h), drop_val(drop_j)
    host_wins = vh >= vj
    winner_id = R["host_id"] if host_wins else u["tg_id"]
    # give both drops to winner as inventory gifts or stars
    async def give_drop(tg_id: int, d: dict):
        usr = None
        async with get_db() as db:
            row = await (await db.execute("SELECT tg_id, username, balance, inventory, games, wins, deposited, free_case_at, friend_case_at, chance_bonus, ton_wallet, cases_opened FROM users WHERE tg_id=?", (tg_id,))).fetchone()
            if not row:
            ...
            await db.commit()
    # both drops to winner
    await give_drop(winner_id, drop_h)
    await give_drop(winner_id, drop_j)
    R["done"] = True
    BATTLES.pop(body.room_id, None)
    u2 = await current_user(authorization, x_telegram_init_data)
    you_win = winner_id == u["tg_id"]
    # joiner is always drop_j, host is drop_h
    return {
        "winner_id": winner_id,
        "you_win": you_win,
        "your_drop": drop_view(drop_j),
        "opp_drop": drop_view(drop_h),
        "balance": u2["balance"],
    }

# ----- TRADE PLAZA -----
TRADE_PLAZA: Dict[int, dict] = {}  # tg_id -> {username, balance, ts}
...
async def trade_join(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    TRADE_PLAZA[u["tg_id"]] = {"username": u.get("username") or str(u["tg_id"]), "balance": u["balance"], "ts": time.time()}
    return {"ok": True}
...
async def trade_plaza(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    now = time.time()
    # prune >10 min
    for tid in list(TRADE_PLAZA.keys()):
        if now - TRADE_PLAZA[tid].get("ts", 0) > 600:
            TRADE_PLAZA.pop(tid, None)
    users = [{"tg_id": tid, "username": v["username"], "balance": v["balance"]} for tid, v in TRADE_PLAZA.items()]
    requests = []
    active = None
    for trid, T in list(TRADES.items()):
        if T.get("done"):
            continue
        if T.get("to_id") == u["tg_id"] and T.get("status") == "open":
            requests.append({"id": trid, "from_name": T.get("from_name") or "Игрок"})
        if u["tg_id"] in (T.get("from_id"), T.get("to_id")):
            active = _trade_view(T, u["tg_id"])
    return {"users": users, "requests": requests, "active": active}

def _trade_view(T: dict, me: int) -> dict:
    mine = T["offer_a"] if T["from_id"] == me else T["offer_b"]
    their = T["offer_b"] if T["from_id"] == me else T["offer_a"]
    return {"id": T["id"], "my": mine, "their": their, "status": T.get("status") or "open"}
...
async def trade_offer(body: TradeOfferRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    if body.to_id not in TRADE_PLAZA:
        raise HTTPException(400, "Игрок не в плазе")
    tid = uuid.uuid4().hex[:10]
    T = {
        "id": tid,
        "from_id": u["tg_id"],
        "from_name": u.get("username") or str(u["tg_id"]),
        "to_id": body.to_id,
        "offer_a": {"stars": 0, "items": [], "item_indices": []},
        "offer_b": {"stars": 0, "items": [], "item_indices": []},
        "status": "open",
        "accepted_a": False,
        "accepted_b": False,
        "done": False,
    ...
    TRADES[tid] = T
    return {"trade_id": tid, "id": tid, "trade": _trade_view(T, u["tg_id"])}
...
async def trade_get(trade_id: str, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    return {"trade": _trade_view(T, u["tg_id"])}
...
async def trade_add(body: TradeAddRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    # reset accept on change
    T["accepted_a"] = False
    T["accepted_b"] = False
    T["status"] = "open"
    if body.stars and body.stars > 0:
        if u["balance"] < body.stars + offer["stars"]:
            raise HTTPException(400, "Недостаточно ⭐")
    ...
    if body.item_index is not None:
        ...
        offer.setdefault("item_indices", []).append(idx)
    return {"trade": _trade_view(T, u["tg_id"])}
...
async def trade_accept(body: TradeIdRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    if T["accepted_a"] and T["accepted_b"]:
        ...
        async def save_u(usr):
            async with get_db() as db:
                await db.execute("UPDATE users SET balance=?, inventory=? WHERE tg_id=?", (usr["balance"], json.dumps(usr["inventory"], ensure_ascii=False), usr["tg_id"]))
                await db.commit()
        ...
        # stars
        if a["balance"] < oa["stars"] or b["balance"] < ob["stars"]:
            raise HTTPException(400, "Не хватает ⭐ у одной из сторон")
        a["balance"] = a["balance"] - oa["stars"] + ob["stars"]
        b["balance"] = b["balance"] - ob["stars"] + oa["stars"]
        # items by removing offered from each and adding to other
        ...
        T["done"] = True
        T["status"] = "done"
        return {"done": True, "trade": _trade_view(T, u["tg_id"])}
    T["status"] = "partial"
    return {"done": False, "trade": _trade_view(T, u["tg_id"])}
