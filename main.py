...
<title>Admin GiftUpgrader</title>
<style>
*{box-sizing:border-box}
body{font-family:system-ui,sans-serif;background:#0b0f1a;color:#e8ecf4;margin:0;padding:16px;max-width:800px;margin-inline:auto}
h1{font-size:18px;margin:0 0 8px}
h2{font-size:14px;color:#f5c542;margin:16px 0 8px}
input,button,select{padding:10px 12px;border-radius:10px;border:1px solid #333;background:#12182a;color:#fff;font-size:14px}
button{background:#3b82f6;border:none;cursor:pointer;font-weight:700}
button.sec{background:#6366f1}
button.ok{background:#16a34a}
button.danger{background:#b91c1c}
.row{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px}
.card{background:#12182a;border:1px solid #222;border-radius:12px;padding:12px;margin-top:8px;font-size:13px;line-height:1.45}
.muted{color:#7a8699;font-size:12px}
.win{color:#4ade80}.lose{color:#f87171}
code{background:#0004;padding:2px 6px;border-radius:6px}
a{color:#60a5fa}
</style></head><body>
<h1>👑 Админка GiftUpgrader</h1>
<p class="muted">Ключ = <code>ADMIN_TOKEN</code> из Render Environment</p>
<div class="row">
  <input id="key" type="password" placeholder="ADMIN_TOKEN" style="flex:2;min-width:160px">
  <button class="sec" onclick="saveKey()">Сохранить ключ</button>
</div>

<h2>Выдать ⭐</h2>
<div class="row">
  <input id="giveUid" placeholder="tg_id игрока" style="flex:1">
  <input id="giveAmt" type="number" placeholder="сумма" value="1000" style="width:120px">
...
<div class="row">
  ...
  <button class="sec" onclick="listUsers()">Топ / все</button>
</div>
<div id="out" class="card muted">Вставь ключ → Топ или tg_id → Смотреть</div>

<h2>Заявки на вывод</h2>
<div class="row">
  <button onclick="loadWd()">Обновить выводы</button>
  <button class="sec" onclick="dbInfo()">DB info</button>
  <button class="ok" onclick="doBackup()">Бэкап в TG</button>
</div>
<div id="wd" class="card muted">—</div>
...
async function api(method,url,body){
  const h={'Accept':'application/json','X-Admin-Key':key()};
  if(body) h['Content-Type']='application/json';
  const r=await fetch(url,{method,headers:h,body:body?JSON.stringify(body):undefined});
  const t=await r.text();
  let j; try{j=JSON.parse(t);}catch(e){throw new Error(t.slice(0,180)||('HTTP '+r.status));}
...
async function giveStars(){
  ...
  try{
    const d=await api('POST','/api/admin/give',{user_id:uid,amount});
    alert('Выдано '+amount+'⭐ → '+uid+(d.balance!=null?(' · баланс '+d.balance):''));
  }catch(e){alert(e.message||e);}
}
async function loadUser(){
  const out=document.getElementById('out');
  const uid=document.getElementById('uid').value.trim();
  if(!uid){out.textContent='Нужен tg_id';return;}
  out.textContent='…';
  try{
    const d=await api('GET','/api/admin/user?user_id='+encodeURIComponent(uid)+'&limit=50');
    const u=d.user||{};
    let h=`<b>@${u.username||'—'} · ${u.tg_id}</b><br>
      Баланс <b class=win>${u.balance}⭐</b> · инв ${u.inventory_count}≈${u.inventory_value}⭐<br>
      Деп ${u.deposited} · игр ${u.games} · побед ${u.wins} · кейсов ${u.cases_opened}<br>
      net <b class="${(u.net_hint||0)>=0?'win':'lose'}">${u.net_hint}⭐</b><br><br><b>Режимы</b><br>`;
    for(const m of (d.by_mode||[])) h+=`${m.game}: ${m.spins} игр, win ${m.wins}, Σ${m.sum_amount}<br>`;
    h+='<br><b>История</b><br>';
    for(const x of (d.history||[]).slice(0,40)){
      const c=x.result==='lose'?'lose':(x.result==='win'||x.result==='gift'?'win':'');
      const t=x.ts?new Date(x.ts*1000).toLocaleString('ru-RU'):'';
      h+=`<div class=${c}>${x.game} ${x.result} · ${x.detail||''} · <b>${x.amount||0}</b><div class=muted>${t}</div></div>`;
    }
    out.innerHTML=h;
...
async function listUsers(){
  ...
  out.textContent='…';
  try{
    const d=await api('GET','/api/admin/users?limit=50');
    let h='<b>Игроки</b> (всего '+(d.total!=null?d.total:'?')+')<br>';
    ...
    if(!(d.users||[]).length) h+='<span class=muted>Пусто — игроки появятся после первого входа в мини-апп</span>';
    out.innerHTML=h;
  }catch(e){out.innerHTML='<span class=lose>'+(e.message||e)+'</span>';}
}
...
async function dbInfo(){
  ...
  try{
    ...
    out.innerHTML='<pre style="white-space:pre-wrap">'+JSON.stringify(d,null,2)+'</pre>';
  }catch(e){out.innerHTML='<span class=lose>'+(e.message||e)+'</span>';}
}
