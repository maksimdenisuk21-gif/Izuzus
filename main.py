...
ADMIN_KEY = (os.getenv("ADMIN_KEY") or os.getenv("ADMIN_TOKEN") or "").strip()
# несколько админов: ADMIN_TG_IDS=111,222,333
_ADMIN_IDS = {ADMIN_TG_ID}
for _x in (os.getenv("ADMIN_TG_IDS") or "").split(","):
    _x = _x.strip()
    if _x.isdigit():
...
HOUSE_EDGE = float(os.getenv("HOUSE_EDGE") or "0.035")  # RTP ≈ 96.5%
MIN_BET = 50
...
CREATE TABLE IF NOT EXISTS trades (
  ...
  creator_id BIGINT,
  taker_id BIGINT,
  give_stars INTEGER DEFAULT 0,
  give_items TEXT DEFAULT '[]',
  want_stars INTEGER DEFAULT 0,
  want_note TEXT DEFAULT '',
    ...
    async with get_db() as db:
        ...
        if USE_POSTGRES:
            migrations = [
                ...
                "ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS method TEXT DEFAULT 'stars'",
                "ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS dest TEXT DEFAULT ''",
                ...
                "ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS amount INTEGER DEFAULT 0",
                "ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS created_at BIGINT DEFAULT 0",
                "ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS wallet TEXT DEFAULT ''",
                "CREATE TABLE IF NOT EXISTS trades (id SERIAL PRIMARY KEY, creator_id BIGINT, taker_id BIGINT, give_stars INTEGER DEFAULT 0, give_items TEXT DEFAULT '[]', want_stars INTEGER DEFAULT 0, want_note TEXT DEFAULT '', status TEXT DEFAULT 'open', created_at INTEGER)",
                "CREATE TABLE IF NOT EXISTS trade_messages (id SERIAL PRIMARY KEY, trade_id INTEGER, tg_id BIGINT, username TEXT, text TEXT, created_at INTEGER)",
                "CREATE TABLE IF NOT EXISTS battles (id SERIAL PRIMARY KEY, host_id BIGINT, host_name TEXT, guest_id BIGINT, guest_name TEXT, case_id TEXT, price INTEGER, host_drop TEXT, guest_drop TEXT, winner_id BIGINT, status TEXT DEFAULT 'open', created_at INTEGER)",
...
CASES = {
    ...
    'star_case_1': {'name': '⭐ STAR CASE I', 'price': 55, 'category': 'stars', 'icon': '⭐', 'color': 'c-starter', 'star_drops': [15, 25, 35, 45, 55, 85, 125, 210], 'star_weights': [22, 20, 18, 14, 12, 8, 4, 2], 'desc': 'RTP ~72% · редко выше цены'},
    'star_case_2': {'name': '⭐ STAR CASE II', 'price': 110, 'category': 'stars', 'icon': '✨', 'color': 'c-starter', 'star_drops': [25, 45, 65, 90, 115, 175, 285, 460], 'star_weights': [22, 20, 18, 14, 12, 8, 4, 2], 'desc': 'RTP ~72%'},
    'star_case_3': {'name': '⭐ STAR CASE III', 'price': 280, 'category': 'stars', 'icon': '🌟', 'color': 'c-pepe', 'star_drops': [60, 100, 150, 220, 285, 430, 720, 1150], 'star_weights': [22, 20, 18, 14, 12, 8, 4, 2], 'desc': 'RTP ~72%'},
    'star_case_4': {'name': '⭐ STAR CASE IV', 'price': 550, 'category': 'stars', 'icon': '💫', 'color': 'c-tg', 'star_drops': [120, 200, 300, 430, 560, 860, 1420, 2550], 'star_weights': [22, 20, 18, 14, 12, 8, 4, 2], 'desc': 'RTP ~72%'},
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    raw = (iv / tv) * 100.0
    # x2 ≈ 48.25% → RTP ~96.5%
    ch = max(0.01, min(78.0, raw * 0.965))
    ch = min(88.0, ch + min(2.0, float(bonus or 0)))
    return round(ch, 2)

def calc_upgrade_display(iv: float, tv: float, bonus: float = 0.0) -> float:
    """На колесе почти как реальный RTP 96–97%."""
    iv = float(iv or 0); tv = float(tv or 1)
    if tv <= 0 or iv <= 0:
        return 0.01
    raw = (iv / tv) * 100.0
    ch = max(0.01, min(80.0, raw * 0.97))
    ch = min(90.0, ch + min(2.0, float(bonus or 0)))
    return round(ch, 2)

def roll_crash_point() -> float:
    r = random.SystemRandom().random()
    # ~3% краш 1.00x, фактор 0.965 → RTP ~96%
    if r < 0.03:
        return 1.0
    p = max(1.01, 0.965 / (1 - r))
...
def mines_mult(mines: int, opened: int) -> float:
    ...
    for i in range(opened):
        ...
        if safe <= 0:
            break
        mult *= remaining / safe
    return max(1.0, mult * (1 - max(HOUSE_EDGE, 0.03)))  # мины ~ RTP 96%+
...
async def current_user(
    ...
    async with get_db() as db:
        ...
        return {
            ...
            "is_admin": int(row[0]) in _ADMIN_IDS,
        }
...
async def _resolve_admin(authorization, x_telegram_init_data, x_admin_key=None):
    me = {"is_admin": False, "tg_id": 0}
    try:
        me = await current_user(authorization, x_telegram_init_data)
    except Exception:
        pass
    require_admin(me, x_admin_key)
    return me

def require_admin(u: dict, x_admin_key: Optional[str] = None):
    if u and u.get("is_admin"):
        return
    key = (x_admin_key or "").strip()
    if ADMIN_KEY and key and hmac.compare_digest(key, ADMIN_KEY):
        return
    raise HTTPException(403, "admin only — зайди своим TG или укажи ADMIN_KEY")

def _admin_key_from(request: Optional[Request] = None, x_admin_key: Optional[str] = None) -> str:
    if x_admin_key:
        return x_admin_key
    return ""
...
def _slot_targets(price: int) -> list:
    ...
    bands = [
        ...
        (0.55, 0.65),  # ~60%
        (0.70, 0.82),  # ~76%
        (0.88, 0.98),  # ~93% почти окуп
        (1.00, 1.15),  # ~окуп / чуть плюс
        (1.35, 1.70),  # плюс
        (2.00, 2.80),  # большой
        (3.50, 4.80),  # джекпот
    ]
...
SLOT_WEIGHTS = [22, 20, 18, 14, 12, 8, 4, 2]  # RTP ~96.5%
...
<title>Admin · Gift Upgrader</title>
<style>
*{box-sizing:border-box}
body{font-family:system-ui,-apple-system,sans-serif;background:#080b14;color:#eef2ff;margin:0;padding:16px;max-width:860px;margin-inline:auto}
h1{font-size:20px;margin:0 0 4px;font-weight:900}
h2{font-size:13px;color:#a8c0ff;margin:20px 0 8px;letter-spacing:.06em;text-transform:uppercase}
input,button,select{padding:11px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.1);background:#12182a;color:#fff;font-size:14px}
button{background:linear-gradient(135deg,#4f7cff,#7a55ff);border:none;cursor:pointer;font-weight:700}
button.sec{background:#1e293b}
button.ok{background:#16a34a}
button.danger{background:#b91c1c}
.row{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px}
.card{background:#12182a;border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:14px;margin-top:8px;font-size:13px;line-height:1.5}
.muted{color:#8b97ad;font-size:12px}
.win{color:#4ade80}.lose{color:#f87171}
code{background:#0005;padding:2px 6px;border-radius:6px}
a{color:#8eb4ff}
.tabs{display:flex;gap:6px;flex-wrap:wrap;margin:12px 0}
.tabs button{font-size:12px;padding:8px 12px}
.pill{display:inline-block;padding:2px 8px;border-radius:999px;background:rgba(79,124,255,.2);font-size:11px;margin-left:6px}
</style></head><body>
<h1>👑 Gift Upgrader</h1>
<p class="muted">Админка · ключ = <code>ADMIN_TOKEN</code> / <code>ADMIN_KEY</code> в Render</p>
<div class="row">
  <input id="key" type="password" placeholder="ADMIN_TOKEN" style="flex:2;min-width:160px">
  <button class="sec" onclick="saveKey()">Сохранить</button>
</div>

<div class="tabs">
  <button onclick="listUsers()">👥 Игроки</button>
  <button onclick="loadWd()">💸 Выводы</button>
  <button onclick="dbInfo()">🗄 DB</button>
  <button class="ok" onclick="doBackup()">💾 Бэкап TG</button>
</div>

<h2>Выдать ⭐</h2>
<div class="row">
  <input id="giveUid" placeholder="tg_id" style="flex:1">
  <input id="giveAmt" type="number" placeholder="сумма" value="1000" style="width:110px">
  <button class="ok" onclick="giveStars()">Выдать</button>
</div>

<h2>Промокод</h2>
<div class="row">
  <input id="promoCode" placeholder="CODE" style="flex:1;text-transform:uppercase">
  <input id="promoStars" type="number" value="100" style="width:90px" title="звёзды">
  <input id="promoUses" type="number" value="50" style="width:90px" title="активаций">
  <button onclick="createPromo()">Создать</button>
</div>

<h2>Игрок / история</h2>
<div class="row">
  <input id="uid" placeholder="tg_id" style="flex:1">
  <button onclick="loadUser()">Смотреть</button>
</div>
<div id="out" class="card muted">Ключ → Игроки или tg_id → Смотреть</div>

<h2>Заявки на вывод</h2>
<div id="wd" class="card muted">Нажми «Выводы»</div>

<script>
const LS='admin_key_v2';
try{document.getElementById('key').value=localStorage.getItem(LS)||'';}catch(e){}
function saveKey(){try{localStorage.setItem(LS,document.getElementById('key').value.trim());alert('OK');}catch(e){}}
...
async function api(method,url,body){
  ...
  if(!r.ok) throw new Error(d.detail||d.message||t||r.status);
  return d;
}
async function giveStars(){
  const uid=parseInt(document.getElementById('giveUid').value,10);
  const amount=parseInt(document.getElementById('giveAmt').value,10);
  if(!uid||!amount){alert('tg_id и сумма');return;}
  try{const d=await api('POST','/api/admin/give',{user_id:uid,amount});alert('OK balance='+d.balance);}catch(e){alert(e.message||e)}
}
async function createPromo(){
  const code=(document.getElementById('promoCode').value||'').trim().toUpperCase();
  const stars=parseInt(document.getElementById('promoStars').value,10)||100;
  const max_uses=parseInt(document.getElementById('promoUses').value,10)||50;
  if(!code){alert('Код');return;}
  try{
    const d=await api('POST','/api/admin/promo',{code,reward_type:'stars',stars,max_uses});
    alert('Промо '+ (d.code||code) +' · ⭐'+stars+' · uses '+max_uses);
  }catch(e){alert(e.message||e)}
}
async function loadUser(){
  const uid=(document.getElementById('uid').value||'').trim();
  const out=document.getElementById('out');
  if(!uid){out.innerHTML='Укажи tg_id';return;}
  out.textContent='…';
  try{
    const d=await api('GET','/api/admin/user?user_id='+encodeURIComponent(uid)+'&limit=50');
    const u=d.user||d;
    let h=`<b>@${u.username||'—'} ${u.tg_id||uid}</b> · <b>${u.balance??'?'}⭐</b>
      · деп ${u.deposited||0} · игр ${u.games||0} · побед ${u.wins||0}<br>`;
    for(const x of (d.history||[])){
      const t=x.created_at?new Date((x.created_at>1e12?x.created_at:x.created_at*1000)).toLocaleString():'';
      const cls=x.result==='win'?'win':'lose';
      h+=`<div class="${cls}">${x.game||''} · ${x.result||''} · ${x.detail||''} · <b>${x.amount||0}</b><div class=muted>${t}</div></div>`;
    }
    out.innerHTML=h;
  }catch(e){out.innerHTML='<span class=lose>'+(e.message||e)+'</span>';}
}
async function listUsers(){
  const out=document.getElementById('out');
  out.textContent='…';
  try{
    const d=await api('GET','/api/admin/users?limit=80');
    let h='<b>Игроки</b> <span class=pill>'+(d.total!=null?d.total:'?')+'</span><br>';
    for(const u of (d.users||[])){
      h+=`<a href=# onclick="document.getElementById('uid').value='${u.tg_id}';document.getElementById('giveUid').value='${u.tg_id}';loadUser();return false">@${u.username||'—'} ${u.tg_id}</a>
        · <b>${u.balance}⭐</b> · деп ${u.deposited} · игр ${u.games}<br>`;
    }
    if(!(d.users||[]).length) h+='<span class=muted>Пусто — зайди в мини-апп своим TG</span>';
    out.innerHTML=h;
  }catch(e){out.innerHTML='<span class=lose>'+(e.message||e)+'</span>';}
}
async function loadWd(){
  ...
  el.textContent='…';
  try{
    const d=await api('GET','/api/admin/withdrawals');
    const list=d.items||d.withdrawals||[];
    if(!list.length){el.innerHTML='Нет заявок';return;}
    el.innerHTML=list.map(w=>{
      const pending=w.status==='pending';
      return `<div style="border-bottom:1px solid #222;padding:8px 0">
        <b>#${w.id}</b> ${w.amount}⭐ · ${w.status} · user ${w.tg_id}<br>
        <code>${w.dest||''}</code>
        ${pending?`<div class=row style="margin-top:6px">
          <button class=ok onclick="wd(${w.id},'approved')">Выдал</button>
          <button class=danger onclick="wd(${w.id},'rejected')">Отклонить</button>
        </div>`:''}
      </div>`;
    }).join('');
  }catch(e){el.innerHTML='<span class=lose>'+(e.message||e)+'</span>';}
}
async function dbInfo(){
  const out=document.getElementById('out');
  out.textContent='…';
  try{
    const d=await api('GET','/api/admin/dbinfo');
    out.innerHTML='<pre style="white-space:pre-wrap;font-size:12px">'+JSON.stringify(d,null,2)+'</pre>';
  }catch(e){out.innerHTML='<span class=lose>'+(e.message||e)+'</span>';}
}
async function doBackup(){
  try{
    const d=await api('POST','/api/admin/backup',{});
    alert('Бэкап: users='+d.users+'\\n'+JSON.stringify(d.sent));
  }catch(e){alert(e.message||e);}
}
async function wd(id,status){
  try{
    await api('POST','/api/admin/withdraw/status',{withdraw_id:id,status});
    loadWd();
  }catch(e){alert(e.message||e);}
}
</script>
...
async def _open_case_inner(body: CaseOpenRequest, authorization, x_telegram_init_data):
    ...
    _RATE_CASE[u["tg_id"]] = now_ts()
    c = CASES.get(body.case_id)
    if not c:
        raise HTTPException(404, "case not found")
    price = int(c.get("price") or 0)
    count = max(1, min(3, int(getattr(body, "count", 1) or 1)))
    # free / share / deposit only x1
    if price == 0 or c.get("require_share") or c.get("require_deposit") or body.case_id in ("free_daily", "free_friend"):
        count = 1
    ...
    if total_price > 0:
        if u["balance"] < total_price:
            raise HTTPException(400, f"Нужно ⭐{total_price} (x{count})")
    ...
    for i in range(count):
        ...
        result = {"success": True, "fair": fair, "allin_lose": bool(drop.get("allin_lose")), "index": i}
        if drop["kind"] == "gift":
            g = drop["gift"]
            ...
            result["stars_earned"] = None
            await add_history(u["tg_id"], "case", "win", f"{c.get('name')}: {inv_item.get('name')}", int(inv_item.get("value") or 0))
        else:
    ...
    # backward compat: first/last drop fields on root
    if last_result:
        if last_result.get("gift"):
            out["gift"] = last_result["gift"]
        if last_result.get("stars_earned") is not None:
            out["stars_earned"] = last_result["stars_earned"]
        out["fair"] = last_result.get("fair")
        out["allin_lose"] = last_result.get("allin_lose")
    return out
...
async def mines_start(body: MinesStartRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    if mines < 3 or mines > 12:
        raise HTTPException(400, "Мины 3–12")
    now = now_ts()
...
async def pvp_list(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    for lid, L in list(PVP.items()):
        ...
        cd_at = L.get("countdown_at")
        cd_left = max(0, int(cd_at) - now_ts()) if cd_at else None
        # если отсчёт вышел и ещё не стартовали — клиент вызовет start
        view = {
            "id": lid,
            "players": pl,
            "player_list": pl,
...
async def pvp_create(body: PvpCreateRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    PVP[lid] = {
        ...
        "started": False,
        "countdown_at": None,  # unix ts when game auto-starts
        "created_at": now_ts(),
    }
    await save_user(u)
...
async def pvp_join(body: PvpJoinRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    # Автостарт: при 2+ игроках система сама даёт отсчёт (не кто-то жмёт «Старт»)
    if len(L["players"]) >= 2 and not L.get("countdown_at"):
        L["countdown_at"] = now_ts() + 12
    await save_user(u)
    return {"ok": True, "balance": u["balance"], "countdown_at": L.get("countdown_at")}
...
async def pvp_start(body: PvpStartRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    for p in players:
        ...
        if p is winner:
            ...
            break
        deg_acc += slice_deg
    payout = clamp_win(int(total * (1 - max(HOUSE_EDGE, 0.05))))  # PvP ~5%, cap MAX_WIN
...
async def my_withdrawals(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        rows = None
        for sql in (
            "SELECT id, amount, method, dest, status, created_at FROM withdrawals WHERE tg_id=? ORDER BY id DESC LIMIT 30",
            "SELECT id, amount, status, created_at FROM withdrawals WHERE tg_id=? ORDER BY id DESC LIMIT 30",
            "SELECT id, amount FROM withdrawals WHERE tg_id=? ORDER BY id DESC LIMIT 30",
        ):
            try:
                rows = await (await db.execute(sql, (u["tg_id"],))).fetchall()
                break
            except Exception as e:
                print("[my/withdrawals]", e)
                rows = None
        items = []
        for r in (rows or []):
            try:
                if len(r) >= 6:
                    items.append({"id": int(r[0]), "amount": int(r[1] or 0), "method": r[2] or "", "dest": r[3] or "", "status": r[4] or "", "ts": r[5]})
                elif len(r) >= 4:
                    items.append({"id": int(r[0]), "amount": int(r[1] or 0), "method": "stars", "dest": "", "status": r[2] or "", "ts": r[3]})
                else:
                    items.append({"id": int(r[0]), "amount": int(r[1] or 0), "method": "stars", "dest": "", "status": "pending", "ts": None})
            except Exception:
                continue
...
async def withdraw(body: WithdrawRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    dest = (body.dest or getattr(body, "username", None) or getattr(body, "wallet", None) or "") or ""
    method = (body.method or "stars") or "stars"
    note = (body.note or "") or ""
    lock = await user_lock(u["tg_id"])
    async with lock:
        async with get_db() as db:
            # актуальный баланс из БД
            brow = await (await db.execute("SELECT balance FROM users WHERE tg_id=?", (u["tg_id"],))).fetchone()
            if not brow:
            ...
            try:
                await db.execute(
                    ...
                    (int(u["tg_id"]), amount, method, str(dest)[:500], str(note)[:500], "pending", now_ts()),
                )
            except Exception as e:
                print("[withdraw insert full]", e)
                try:
                    await db.execute(
                        "INSERT INTO withdrawals(tg_id,amount,status,created_at) VALUES(?,?,?,?)",
                        (int(u["tg_id"]), amount, "pending", now_ts()),
                    )
                except Exception as e2:
                    print("[withdraw insert minimal]", e2)
                    raise HTTPException(500, f"Не удалось создать заявку: {e2}")
            new_bal = bal - amount
            await db.execute("UPDATE users SET balance=? WHERE tg_id=?", (new_bal, int(u["tg_id"])))
            await db.commit()
        return {"ok": True, "success": True, "balance": new_bal, "message": "Заявка отправлена админу"}
...
async def promo_activate(
    body: Optional[PromoActivateRequest] = None,
    code: Optional[str] = Query(None),
    authorization: Optional[str] = Header(None),
    x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"),
):
    u = await current_user(authorization, x_telegram_init_data)
    raw = (code or (body.code if body else "") or "").strip().upper()
    if not raw:
        raise HTTPException(400, "Введите промокод")
    async with get_db() as db:
        row = await (await db.execute("SELECT code, stars, max_uses, uses FROM promos WHERE code=?", (raw,))).fetchone()
        if not row:
            raise HTTPException(400, "Неверный промокод")
        used = await (await db.execute("SELECT 1 FROM promo_uses WHERE code=? AND tg_id=?", (raw, u["tg_id"]))).fetchone()
        if used:
        ...
        try:
            await db.execute("INSERT INTO promo_uses(code,tg_id) VALUES(?,?)", (raw, u["tg_id"]))
        except Exception:
            raise HTTPException(400, "Уже использован")
        await db.execute("UPDATE promos SET uses=uses+1 WHERE code=?", (raw,))
        await db.commit()
    u["balance"] = int(u.get("balance") or 0) + stars
    await save_user(u)
    await add_history(u["tg_id"], "promo", "win", f"Промо {raw}", stars)
    return {"ok": True, "success": True, "stars": stars, "balance": u["balance"]}
...
async def leaderboard():
    """Топ всех игроков. Устойчиво к NULL и старым схемам."""
    empty = {"total_users": 0, "by_balance": [], "by_wins": [], "by_cases": [], "by_new": [], "all": []}
    try:
        async with get_db() as db:
            total_row = await (await db.execute("SELECT COUNT(*) FROM users")).fetchone()
            total = int(total_row[0] or 0) if total_row else 0
            # минимальный SELECT — только обязательные поля
            try:
                all_rows = await (await db.execute(
                    "SELECT tg_id, username, balance, wins, cases_opened FROM users"
                )).fetchall()
            except Exception as e1:
                print("[leaderboard] full select fail", e1)
                all_rows = await (await db.execute(
        ...
        for r in (all_rows or []):
            try:
                tg = int(r[0] or 0)
                name = str(r[1] or ("id:" + str(tg)))
                bal = int(r[2] or 0)
                wins = int(r[3] or 0) if len(r) > 3 else 0
                cases = int(r[4] or 0) if len(r) > 4 else 0
                rows.append({"name": name, "balance": bal, "wins": wins, "cases": cases, "tg_id": tg})
            except Exception:
                continue
        by_bal = sorted(rows, key=lambda x: x["balance"], reverse=True)
        by_wins = sorted(rows, key=lambda x: x["wins"], reverse=True)
        by_cases = sorted(rows, key=lambda x: x["cases"], reverse=True)
        return {
            "total_users": total or len(rows),
            "by_balance": by_bal,
            "by_wins": by_wins,
            "by_cases": by_cases,
            "by_new": by_bal,
...
class CraftRequest(BaseModel):
    item_ids: list = []  # inventory item ids (min 3)
    item_indices: list = []  # fallback: indices in inventory
...
async def craft_items(
    ...
    """Craft 3+ inventory gifts into one. House edge ~12% (RTP ~88%)."""
    u = await current_user(authorization, x_telegram_init_data)
    ...
    async with get_db() as db:
        ...
        # Balanced craft: EV ~88% of total, range ~0.1x .. 10x
        # bands: (weight, min_mult, max_mult)
        bands = [
            (0.38, 0.12, 0.32),   # small return
            (0.28, 0.32, 0.55),   # modest
            (0.16, 0.55, 0.85),   # medium
            (0.09, 0.85, 1.15),   # near break-even
            (0.055, 1.15, 2.0),   # good
            (0.025, 2.0, 4.0),    # big
            (0.008, 4.0, 7.0),    # huge
            (0.002, 7.0, 10.0),   # jackpot
        ]
        r = random.random()
        acc = 0.0
        lo, hi = 0.12, 0.32
        ...
        pool = list(GIFTS_FLAT) if GIFTS_FLAT else []
        if not pool:
            pool = [{"name": "Подарок", "value": target, "rarity": "Common", "emoji": "🎁", "sn": "gift"}]
        # pick gift closest to target value (among candidates in range)
        ...
        for g, w in zip(top, weights):
            ...
            if rr <= accw:
                ...
                break
        result = {
            "id": f"craft_{u['tg_id']}_{int(time.time())}_{random.randint(1000,9999)}",
            "name": chosen.get("name") or "Подарок",
            "value": int(chosen.get("value") or target),
            "rarity": chosen.get("rarity") or "Common",
        ...
        try:
            await db.execute(
                "INSERT INTO history (tg_id, game, result, detail, amount, created_at) VALUES (?,?,?,?,?,?)",
                (u["tg_id"], "craft", "ok", f"{len(selected)}→{result['name']}", result["value"], int(time.time())),
            )
        except Exception:
            pass
        await db.commit()
        return {"success": True, "gift": result, "spent_value": total, "inventory": remaining}

# ----- admin -----
@app.post("/api/admin/give")
async def admin_give(
    body: AdminGiveRequest,
    authorization: Optional[str] = Header(None),
    x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"),
    ...
    try:
        ...
        me = {"is_admin": False}
    require_admin(me, x_admin_key)
    uid = int(body.user_id or body.tg_id or 0)
    amt = int(body.amount or 0)
    if not uid:
        raise HTTPException(400, "user_id")
    if amt == 0:
    ...
    try:
        async with get_db() as db:
            ...
            if not row:
                if USE_POSTGRES:
                    await db.execute(
                        "INSERT INTO users(tg_id,username,balance,inventory,games,wins,deposited,free_case_at,friend_case_at,chance_bonus,ton_wallet,cases_opened,created_at) "
                        "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT (tg_id) DO UPDATE SET balance = users.balance + EXCLUDED.balance",
                        (uid, f"user_{uid}", max(0, amt), "[]", 0, 0, 0, 0, 0, 0.0, "", 0, now_ts()),
                    )
                else:
                    await db.execute(
                        "INSERT OR REPLACE INTO users(tg_id,username,balance,inventory,games,wins,deposited,free_case_at,friend_case_at,chance_bonus,ton_wallet,cases_opened,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (uid, f"user_{uid}", max(0, amt), "[]", 0, 0, 0, 0, 0, 0.0, "", 0, now_ts()),
                    )
                await db.commit()
                row2 = await (await db.execute("SELECT balance FROM users WHERE tg_id=?", (uid,))).fetchone()
                new_bal = int(row2[0] or 0) if row2 else max(0, amt)
            else:
                ...
                await db.commit()
        return {"ok": True, "success": True, "balance": new_bal, "user_id": uid, "amount": amt}
    except Exception as e:
        print("[admin/give]", e)
        raise HTTPException(500, f"give failed: {type(e).__name__}: {e}")

@app.post("/api/admin/take")
async def admin_take(body: AdminTakeRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"), x_admin_key: Optional[str] = Header(None, alias="X-Admin-Key")):
    me = None
    try:
        me = await current_user(authorization, x_telegram_init_data)
    except Exception:
...
async def admin_promo(body: PromoCreateRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"),
    ...
    stars = max(1, int(body.stars or 0))
    max_uses = max(1, int(body.max_uses or 1))
    async with get_db() as db:
        # upsert: create or update stars/max_uses (keep uses count)
        row = await (await db.execute("SELECT code FROM promos WHERE code=?", (code,))).fetchone()
        if row:
            await db.execute(
                "UPDATE promos SET reward_type=?, stars=?, max_uses=? WHERE code=?",
                (body.reward_type or "stars", stars, max_uses, code),
            )
        else:
            await db.execute(
                "INSERT INTO promos(code,reward_type,stars,max_uses,uses) VALUES(?,?,?,?,0)",
                (code, body.reward_type or "stars", stars, max_uses),
            )
        await db.commit()
    return {"ok": True, "code": code, "stars": stars, "max_uses": max_uses}

@app.get("/api/admin/withdrawals")
async def admin_withdrawals(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"), x_admin_key: Optional[str] = Header(None, alias="X-Admin-Key")):
    me = None
    try:
        me = await current_user(authorization, x_telegram_init_data)
    except Exception:
    ...
    try:
        async with get_db() as db:
            # добить колонки на старых Neon-таблицах
            if USE_POSTGRES:
                for col_sql in (
                    "ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS method TEXT DEFAULT 'stars'",
                    "ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS dest TEXT DEFAULT ''",
            ...
            rows = None
            for sql in (
                "SELECT id, tg_id, amount, method, dest, note, status, created_at FROM withdrawals ORDER BY id DESC LIMIT 200",
                "SELECT id, tg_id, amount, COALESCE(method,''), COALESCE(dest,''), COALESCE(note,''), COALESCE(status,'pending'), created_at FROM withdrawals ORDER BY id DESC LIMIT 200",
                "SELECT id, tg_id, amount, status, created_at FROM withdrawals ORDER BY id DESC LIMIT 200",
                "SELECT id, tg_id, amount FROM withdrawals ORDER BY id DESC LIMIT 200",
            ):
                try:
                    rows = await (await db.execute(sql)).fetchall()
                    break
                except Exception as e1:
                    print("[admin/withdrawals] try", e1)
                    rows = None
            if rows is None:
                rows = []
        for r in (rows or []):
            try:
                ...
                if n >= 8:
                    items.append({
                        "id": int(r[0]), "tg_id": int(r[1] or 0), "amount": int(r[2] or 0),
                        "method": str(r[3] or "stars"), "dest": str(r[4] or ""), "note": str(r[5] or ""),
                        "status": str(r[6] or "pending"), "ts": r[7],
                    })
                elif n >= 5:
                    items.append({
                        "id": int(r[0]), "tg_id": int(r[1] or 0), "amount": int(r[2] or 0),
                        "method": "stars", "dest": "", "note": "",
                        "status": str(r[3] or "pending"), "ts": r[4] if n > 4 else None,
                    })
                else:
                    items.append({
                        "id": int(r[0]), "tg_id": int(r[1] or 0), "amount": int(r[2] or 0),
                        "method": "stars", "dest": "", "note": "", "status": "pending", "ts": None,
                    })
            except Exception:
                continue
        items.sort(key=lambda x: (0 if x["status"] == "pending" else 1, -(x["id"] or 0)))
    except Exception as e:
        err = str(e)
        print("[admin/withdrawals]", e)
    return {"items": items, "withdrawals": items, "list": items, "count": len(items), "error": err}

@app.post("/api/admin/withdraw/status")
async def admin_wd_status(body: AdminWithdrawStatusRequest, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"), x_admin_key: Optional[str] = Header(None, alias="X-Admin-Key")):
    me = None
    try:
        me = await current_user(authorization, x_telegram_init_data)
    except Exception:
...
async def admin_dbinfo(
    ...
    try:
        ...
        me = {"is_admin": False}
    require_admin(me, x_admin_key)
    info = {"postgres": USE_POSTGRES, "has_admin_key": bool(ADMIN_KEY)}
    try:
        async with get_db() as db:
            n = await (await db.execute("SELECT COUNT(*) FROM users")).fetchone()
            w = await (await db.execute("SELECT COUNT(*) FROM withdrawals")).fetchone()
            wp = await (await db.execute("SELECT COUNT(*) FROM withdrawals WHERE status='pending'")).fetchone()
            info["users"] = int(n[0] or 0) if n else 0
            info["withdrawals"] = int(w[0] or 0) if w else 0
            info["pending_withdrawals"] = int(wp[0] or 0) if wp else 0
            sample = await (await db.execute(
                "SELECT tg_id, username, balance, created_at FROM users ORDER BY created_at DESC LIMIT 8"
            )).fetchall()
            info["newest"] = [
                {"tg_id": int(r[0]), "username": r[1], "balance": int(r[2] or 0), "created_at": r[3]}
                for r in (sample or [])
            ]
    except Exception as e:
...
async def admin_backup(
    ...
    try:
        ...
        me = {"is_admin": False}
    require_admin(me, x_admin_key)
    return await _send_backup_to_admins()
...
async def _send_backup_to_admins():
    ...
    async with get_db() as db:
        users = await (await db.execute(
            "SELECT tg_id, username, balance, deposited, games, wins, cases_opened, created_at FROM users ORDER BY tg_id"
        )).fetchall()
        wds = await (await db.execute(
            "SELECT id, tg_id, amount, method, dest, status, created_at FROM withdrawals ORDER BY id DESC LIMIT 500"
        )).fetchall()
    payload = {
        "ts": now_ts(),
        ...
        "users": [
            {
                "tg_id": int(r[0]), "username": r[1], "balance": int(r[2] or 0),
                "deposited": int(r[3] or 0), "games": int(r[4] or 0),
                "wins": int(r[5] or 0), "cases": int(r[6] or 0), "created_at": r[7],
            }
            for r in (users or [])
        ],
        "withdrawals": [
            {"id": r[0], "tg_id": r[1], "amount": r[2], "method": r[3], "dest": r[4], "status": r[5], "ts": r[6]}
            for r in (wds or [])
        ],
    }
    path = "/tmp/giftupgrader_backup.json"
    with open(path, "w", encoding="utf-8") as f:
        f.write(json.dumps(payload, ensure_ascii=False, indent=2))
    sent = []
    for aid in sorted(_ADMIN_IDS):
        try:
            # curl multipart — стабильнее urllib
            proc = await asyncio.create_subprocess_exec(
                "curl", "-sS", "-X", "POST",
                f"https://api.telegram.org/bot{BOT_TOKEN}/sendDocument",
                "-F", f"chat_id={aid}",
                "-F", f"caption=Backup GiftUpgrader · users={payload['users_count']} · ts={payload['ts']}",
                "-F", f"document=@{path};filename=backup.json;type=application/json",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            out, err = await asyncio.wait_for(proc.communicate(), timeout=45)
            ok = proc.returncode == 0
            body = (out or b"").decode("utf-8", "ignore")[:300]
            if not ok:
                ...
                proc2 = await asyncio.create_subprocess_exec(
                    ...
                    "-d", f"chat_id={aid}",
                    "-d", f"text=Backup users={payload['users_count']}. File failed: {(err or b'').decode()[:120]}",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                )
                await asyncio.wait_for(proc2.communicate(), timeout=20)
            sent.append({"admin": aid, "ok": ok, "resp": body[:200]})
        except Exception as e:
            sent.append({"admin": aid, "ok": False, "error": str(e)})
    return {"ok": True, "users": payload["users_count"], "sent": sent}


async def _backup_loop():
    """Раз в 12ч бэкап админам (если BOT_TOKEN есть)."""
    await asyncio.sleep(120)
    while True:
        try:
            if BOT_TOKEN and _ADMIN_IDS:
                await _send_backup_to_admins()
                print("[backup] auto sent")
        except Exception as e:
            print("[backup] auto fail", e)
...
async def admin_user(
    ...
    """Карточка игрока: баланс, деп, инвентарь, история по режимам."""
    me = None
    try:
        me = await current_user(authorization, x_telegram_init_data)
    except Exception:
    ...
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT tg_id, username, balance, inventory, games, wins, deposited, chance_bonus, cases_opened, created_at FROM users WHERE tg_id=?",
            (user_id,),
        )).fetchone()
        if not row:
        ...
        try:
            ...
            inv = []
        inv_value = sum(int(x.get("value") or 0) for x in inv if isinstance(x, dict))
        hist = await (await db.execute(
            "SELECT game, result, detail, amount, created_at FROM history WHERE tg_id=? ORDER BY id DESC LIMIT ?",
            (user_id, max(1, min(100, int(limit))),),
        )).fetchall()
        by_game = await (await db.execute(
            """SELECT game,
                      COUNT(*) as n,
                      SUM(CASE WHEN result IN ('win','gift') THEN 1 ELSE 0 END) as wins,
                      COALESCE(SUM(amount),0) as sum_amt
               FROM history WHERE tg_id=? GROUP BY game""",
            (user_id,),
        )).fetchall()
        wds = await (await db.execute(
            "SELECT id, amount, method, dest, status, created_at FROM withdrawals WHERE tg_id=? ORDER BY id DESC LIMIT 20",
            (user_id,),
        )).fetchall()
        deps = await (await db.execute(
            "SELECT id, amount_ton, credited, created_at FROM ton_deposits WHERE tg_id=? ORDER BY created_at DESC LIMIT 15",
            (user_id,),
        )).fetchall()
    modes = [
        {
            ...
            "spins": int(r[1] or 0),
            "wins": int(r[2] or 0),
            "sum_amount": int(r[3] or 0),
        }
        for r in (by_game or [])
    ]
    history = [
        {"game": r[0], "result": r[1], "detail": r[2], "amount": int(r[3] or 0), "ts": r[4]}
        for r in (hist or [])
    ]
    return {
        "user": {
            ...
            "created_at": row[9],
            "net_hint": int(row[2] or 0) + inv_value - int(row[6] or 0),
        },
        "by_mode": modes,
        "history": history,
        "withdrawals": [
            {"id": r[0], "amount": r[1], "method": r[2], "dest": r[3], "status": r[4], "ts": r[5]}
            for r in (wds or [])
        ],
        "ton_deposits": [
            {"id": r[0], "amount_ton": float(r[1] or 0), "credited": bool(r[2]), "ts": r[3]}
            for r in (deps or [])
        ],
    }
...
async def admin_users(
    ...
    async with get_db() as db:
        ...
        if (q or "").strip().isdigit():
            rows = await (await db.execute(
                "SELECT tg_id, username, balance, deposited, games, wins, cases_opened, created_at FROM users WHERE tg_id=? LIMIT 1",
                (int(q.strip()),),
            )).fetchall()
        elif (q or "").strip():
            like = "%" + q.strip().replace("%", "") + "%"
            rows = await (await db.execute(
                "SELECT tg_id, username, balance, deposited, games, wins, cases_opened, created_at FROM users WHERE username LIKE ? ORDER BY balance DESC LIMIT ?",
                (like, lim),
            )).fetchall()
        else:
            # новички сверху
            rows = await (await db.execute(
                "SELECT tg_id, username, balance, deposited, games, wins, cases_opened, created_at FROM users ORDER BY created_at DESC LIMIT ?",
                (lim,),
            )).fetchall()
    return {
        "total": total,
...
async def admin_stats(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"), x_admin_key: Optional[str] = Header(None, alias="X-Admin-Key")):
    me = None
    try:
        me = await current_user(authorization, x_telegram_init_data)
    except Exception:
...
# -------------------- TRADE (2-sided Deal) --------------------
_TRADE_CD = {}  # tg_id -> last action ts
_TRADE_RATE = {}  # tg_id -> [ts,...]
TRADE_PLAZA: Dict[int, float] = {}  # tg_id -> last heartbeat ts
PLAZA_TTL = 120  # сек в плазе без heartbeat


def _trade_rate(tg_id: int, limit: int = 12, window: int = 60):
    now = now_ts()
    arr = [t for t in _TRADE_RATE.get(tg_id, []) if now - t < window]
    if len(arr) >= limit:
        raise HTTPException(429, "Слишком много действий, подожди")
    arr.append(now)
    _TRADE_RATE[tg_id] = arr

async def _ensure_trade_tables(db):
    if USE_POSTGRES:
        stmts = [
            """CREATE TABLE IF NOT EXISTS trade_rooms (
                ...
                a_stars INTEGER DEFAULT 0, b_stars INTEGER DEFAULT 0,
                a_items TEXT DEFAULT '[]', b_items TEXT DEFAULT '[]',
                a_ready INTEGER DEFAULT 0, b_ready INTEGER DEFAULT 0,
                a_confirm INTEGER DEFAULT 0, b_confirm INTEGER DEFAULT 0,
                status TEXT DEFAULT 'invite',
                created_at INTEGER
            )""",
            """CREATE TABLE IF NOT EXISTS trade_messages (
                id SERIAL PRIMARY KEY,
                trade_id INTEGER, tg_id BIGINT, username TEXT, text TEXT, created_at INTEGER
            )""",
        ]
        for s in stmts:
            try:
                ...
                print("[trade schema]", e)
    else:
        try:
            await db.execute("""CREATE TABLE IF NOT EXISTS trade_rooms (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def _room_public(r, me_id: int):
    a_id, b_id = int(r[1] or 0), int(r[2] or 0)
    return {
        "id": int(r[0]),
        "a_id": a_id, "b_id": b_id,
        "a_name": r[3] or "Player", "b_name": r[4] or "Player",
        "a_stars": int(r[5] or 0), "b_stars": int(r[6] or 0),
        "a_items": _parse_items(r[7]), "b_items": _parse_items(r[8]),
        "a_ready": bool(r[9]), "b_ready": bool(r[10]),
        "a_confirm": bool(r[11]), "b_confirm": bool(r[12]),
        "status": r[13] or "invite",
        "ts": r[14],
        "you_are": "a" if me_id == a_id else ("b" if me_id == b_id else None),
    }

class TradeInviteBody(BaseModel):
    to_user_id: int

class TradeRoomBody(BaseModel):
    trade_id: int
...
async def trade_plaza(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    """Только те, кто нажал «Войти в плазу» (heartbeat)."""
    me = await current_user(authorization, x_telegram_init_data)
    now = now_ts()
    # purge stale
    dead = [k for k, t in TRADE_PLAZA.items() if now - t > PLAZA_TTL]
    for k in dead:
        TRADE_PLAZA.pop(k, None)
    ids = [k for k, t in TRADE_PLAZA.items() if k != int(me["tg_id"]) and now - t <= PLAZA_TTL]
    people = []
    async with get_db() as db:
        await _ensure_trade_tables(db)
        for tid in ids[:40]:
            r = await (await db.execute(
                "SELECT tg_id, username, balance, inventory FROM users WHERE tg_id=?", (tid,)
            )).fetchone()
            if not r:
                continue
            inv = _parse_items(r[3])
            preview = []
            for it in inv[:6]:
                if not isinstance(it, dict):
                    continue
                preview.append({
                    "name": it.get("name"), "value": int(it.get("value") or 0),
                    "img": it.get("img") or "", "emoji": it.get("emoji") or "🎁",
                })
                if len(preview) >= 4:
                    break
            people.append({
                "tg_id": int(r[0]), "username": r[1] or "Player",
                "balance": int(r[2] or 0), "items_count": len(inv), "preview": preview,
            })
    people.sort(key=lambda x: -x["balance"])
    return {"people": people, "me": me["tg_id"], "in_plaza": int(me["tg_id"]) in TRADE_PLAZA}

@app.post("/api/trade/plaza/enter")
async def trade_plaza_enter(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    me = await current_user(authorization, x_telegram_init_data)
    TRADE_PLAZA[int(me["tg_id"])] = now_ts()
    return {"ok": True, "in_plaza": True}

@app.post("/api/trade/plaza/leave")
async def trade_plaza_leave(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    me = await current_user(authorization, x_telegram_init_data)
    TRADE_PLAZA.pop(int(me["tg_id"]), None)
    return {"ok": True, "in_plaza": False}

@app.post("/api/trade/plaza/ping")
async def trade_plaza_ping(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    me = await current_user(authorization, x_telegram_init_data)
    if int(me["tg_id"]) in TRADE_PLAZA:
        TRADE_PLAZA[int(me["tg_id"])] = now_ts()
    return {"ok": True, "in_plaza": int(me["tg_id"]) in TRADE_PLAZA}

@app.post("/api/trade/invite")
async def trade_invite(body: TradeInviteBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    _trade_rate(u["tg_id"])
    to_id = int(body.to_user_id)
    if to_id == int(u["tg_id"]):
        raise HTTPException(400, "Нельзя трейдить с собой")
    now = now_ts()
    if to_id not in TRADE_PLAZA or now - TRADE_PLAZA.get(to_id, 0) > PLAZA_TTL:
        raise HTTPException(400, "Игрок не в плазе")
    if int(u["tg_id"]) not in TRADE_PLAZA or now - TRADE_PLAZA.get(int(u["tg_id"]), 0) > PLAZA_TTL:
        raise HTTPException(400, "Сначала войди в плазу")
    # cooldown
    last = _TRADE_CD.get(u["tg_id"], 0)
    if now_ts() - last < 5:
        raise HTTPException(429, "Подожди 5 сек")
    _TRADE_CD[u["tg_id"]] = now_ts()
    async with get_db() as db:
        await _ensure_trade_tables(db)
        # max 3 active rooms
        cnt = await (await db.execute(
            "SELECT COUNT(*) FROM trade_rooms WHERE status IN ('invite','active','confirm') AND (a_id=? OR b_id=?)",
            (u["tg_id"], u["tg_id"]),
        )).fetchone()
        if cnt and int(cnt[0] or 0) >= 3:
            raise HTTPException(400, "Слишком много активных трейдов (макс 3)")
        other = await (await db.execute("SELECT tg_id, username FROM users WHERE tg_id=?", (to_id,))).fetchone()
        if not other:
            raise HTTPException(404, "Игрок не найден")
        await db.execute(
            "INSERT INTO trade_rooms(a_id,b_id,a_name,b_name,a_stars,b_stars,a_items,b_items,a_ready,b_ready,a_confirm,b_confirm,status,created_at) VALUES(?,?,?,?,0,0,'[]','[]',0,0,0,0,'invite',?)",
            (u["tg_id"], to_id, u.get("username") or "Player", other[1] or "Player", now_ts()),
        )
        await db.commit()
        row = await (await db.execute(
            "SELECT id FROM trade_rooms WHERE a_id=? AND b_id=? ORDER BY id DESC LIMIT 1",
            (u["tg_id"], to_id),
        )).fetchone()
    return {"ok": True, "trade_id": int(row[0]) if row else 0}

@app.get("/api/trade/my")
async def trade_my(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        await _ensure_trade_tables(db)
        rows = await (await db.execute(
            "SELECT id,a_id,b_id,a_name,b_name,a_stars,b_stars,a_items,b_items,a_ready,b_ready,a_confirm,b_confirm,status,created_at FROM trade_rooms WHERE (a_id=? OR b_id=?) AND status IN ('invite','active','confirm') ORDER BY id DESC LIMIT 20",
            (u["tg_id"], u["tg_id"]),
        )).fetchall()
    return {"rooms": [_room_public(r, int(u["tg_id"])) for r in (rows or [])]}

@app.get("/api/trade/room")
async def trade_room(trade_id: int = Query(...), authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        await _ensure_trade_tables(db)
        row = await (await db.execute(
            "SELECT id,a_id,b_id,a_name,b_name,a_stars,b_stars,a_items,b_items,a_ready,b_ready,a_confirm,b_confirm,status,created_at FROM trade_rooms WHERE id=?",
            (trade_id,),
        )).fetchone()
    if not row:
        raise HTTPException(404, "Нет комнаты")
    if int(u["tg_id"]) not in (int(row[1] or 0), int(row[2] or 0)):
        raise HTTPException(403, "Не твоя комната")
    return {"room": _room_public(row, int(u["tg_id"]))}

@app.post("/api/trade/accept_invite")
async def trade_accept_invite(body: TradeRoomBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    _trade_rate(u["tg_id"])
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT id,a_id,b_id,status FROM trade_rooms WHERE id=?", (body.trade_id,)
        )).fetchone()
        if not row:
            raise HTTPException(404, "trade")
        if int(row[2] or 0) != int(u["tg_id"]):
            raise HTTPException(403, "Приглашение не тебе")
        if (row[3] or "") != "invite":
            raise HTTPException(400, "Уже не invite")
        await db.execute("UPDATE trade_rooms SET status='active' WHERE id=? AND status='invite'", (body.trade_id,))
        await db.commit()
    return {"ok": True}

@app.post("/api/trade/decline")
async def trade_decline(body: TradeRoomBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT id,a_id,b_id,status FROM trade_rooms WHERE id=?", (body.trade_id,)
        ...
        if not row:
            raise HTTPException(404, "trade")
        if int(u["tg_id"]) not in (int(row[1] or 0), int(row[2] or 0)):
            raise HTTPException(403, "Не твоя")
        await db.execute("UPDATE trade_rooms SET status='declined' WHERE id=?", (body.trade_id,))
        await db.commit()
    return {"ok": True}

@app.post("/api/trade/set")
async def trade_set(body: TradeSetBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    """Положить ⭐ + NFT со своей стороны (ещё не эскроу до confirm)."""
    u = await current_user(authorization, x_telegram_init_data)
    _trade_rate(u["tg_id"])
    stars = max(0, int(body.stars or 0))
    if stars > 50000:
        raise HTTPException(400, "Слишком много ⭐ за раз")
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT id,a_id,b_id,status FROM trade_rooms WHERE id=?", (body.trade_id,)
        )).fetchone()
        if not row:
            raise HTTPException(404, "trade")
        if (row[3] or "") not in ("active", "confirm"):
            raise HTTPException(400, "Сначала оба должны быть в сделке")
        a_id, b_id = int(row[1]), int(row[2])
        me = int(u["tg_id"])
        if me not in (a_id, b_id):
            raise HTTPException(403, "Не твоя")
        if stars and int(u["balance"]) < stars:
            raise HTTPException(400, "Недостаточно ⭐")
        inv = list(u["inventory"] or [])
        taken = []
        for idx in sorted(set(body.item_indices or []), reverse=True):
            if idx < 0 or idx >= len(inv):
                raise HTTPException(400, "Нет предмета")
            taken.append(inv[idx])
        if len(taken) > 8:
            raise HTTPException(400, "Макс 8 NFT со стороны")
        # reset ready/confirm when offer changes
        if me == a_id:
            await db.execute(
                "UPDATE trade_rooms SET a_stars=?, a_items=?, a_ready=0, b_ready=0, a_confirm=0, b_confirm=0, status='active' WHERE id=?",
                (stars, json.dumps(taken, ensure_ascii=False), body.trade_id),
            )
        else:
            await db.execute(
                "UPDATE trade_rooms SET b_stars=?, b_items=?, a_ready=0, b_ready=0, a_confirm=0, b_confirm=0, status='active' WHERE id=?",
                (stars, json.dumps(taken, ensure_ascii=False), body.trade_id),
            )
        await db.commit()
    return {"ok": True}

@app.post("/api/trade/ready")
async def trade_ready(body: TradeRoomBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    """Кнопка Deal? 🤝"""
    u = await current_user(authorization, x_telegram_init_data)
    _trade_rate(u["tg_id"])
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT id,a_id,b_id,a_stars,b_stars,a_items,b_items,a_ready,b_ready,status FROM trade_rooms WHERE id=?",
            (body.trade_id,),
        )).fetchone()
        if not row or (row[9] or "") not in ("active", "confirm"):
            raise HTTPException(400, "Не активен")
        me = int(u["tg_id"])
        a_id, b_id = int(row[1]), int(row[2])
        if me not in (a_id, b_id):
            raise HTTPException(403, "Не твоя")
        # must put something
        if me == a_id:
            if int(row[3] or 0) <= 0 and not _parse_items(row[5]):
                raise HTTPException(400, "Положи ⭐ или NFT")
            await db.execute("UPDATE trade_rooms SET a_ready=1, a_confirm=0, b_confirm=0 WHERE id=?", (body.trade_id,))
        else:
            if int(row[4] or 0) <= 0 and not _parse_items(row[6]):
                raise HTTPException(400, "Положи ⭐ или NFT")
            await db.execute("UPDATE trade_rooms SET b_ready=1, a_confirm=0, b_confirm=0 WHERE id=?", (body.trade_id,))
        await db.commit()
        row2 = await (await db.execute(
            "SELECT a_ready,b_ready FROM trade_rooms WHERE id=?", (body.trade_id,)
        )).fetchone()
        if row2 and int(row2[0] or 0) and int(row2[1] or 0):
            await db.execute("UPDATE trade_rooms SET status='confirm' WHERE id=?", (body.trade_id,))
            await db.commit()
    return {"ok": True}

@app.post("/api/trade/unready")
async def trade_unready(body: TradeRoomBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        row = await (await db.execute("SELECT id,a_id,b_id FROM trade_rooms WHERE id=?", (body.trade_id,))).fetchone()
        if not row:
            raise HTTPException(404, "trade")
        me = int(u["tg_id"])
        if me == int(row[1]):
            await db.execute("UPDATE trade_rooms SET a_ready=0, a_confirm=0, b_confirm=0, status='active' WHERE id=?", (body.trade_id,))
        elif me == int(row[2]):
            await db.execute("UPDATE trade_rooms SET b_ready=0, a_confirm=0, b_confirm=0, status='active' WHERE id=?", (body.trade_id,))
        else:
            raise HTTPException(403, "Не твоя")
        await db.commit()
    return {"ok": True}
...
async def trade_confirm(body: TradeRoomBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    """Второе окно: оба жмут «Согласен» → обмен."""
    u = await current_user(authorization, x_telegram_init_data)
    _trade_rate(u["tg_id"], limit=8)
    fee_rate = 0.03
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT id,a_id,b_id,a_stars,b_stars,a_items,b_items,a_ready,b_ready,a_confirm,b_confirm,status FROM trade_rooms WHERE id=?",
            (body.trade_id,),
        )).fetchone()
        if not row:
            raise HTTPException(404, "trade")
        if (row[11] or "") != "confirm":
            raise HTTPException(400, "Оба должны нажать Deal 🤝")
        if not (int(row[7] or 0) and int(row[8] or 0)):
            raise HTTPException(400, "Не оба ready")
        me = int(u["tg_id"])
        a_id, b_id = int(row[1]), int(row[2])
        if me == a_id:
            await db.execute("UPDATE trade_rooms SET a_confirm=1 WHERE id=?", (body.trade_id,))
        elif me == b_id:
            await db.execute("UPDATE trade_rooms SET b_confirm=1 WHERE id=?", (body.trade_id,))
        else:
            raise HTTPException(403, "Не твоя")
        await db.commit()
        row2 = await (await db.execute(
            "SELECT id,a_id,b_id,a_stars,b_stars,a_items,b_items,a_confirm,b_confirm,status FROM trade_rooms WHERE id=?",
            (body.trade_id,),
        )).fetchone()
        if not (int(row2[7] or 0) and int(row2[8] or 0)):
            return {"ok": True, "waiting": True, "message": "Ждём подтверждение второй стороны"}
        # EXECUTE SWAP
        a_stars, b_stars = int(row2[3] or 0), int(row2[4] or 0)
        a_items, b_items = _parse_items(row2[5]), _parse_items(row2[6])
        # lock room first
        await db.execute("UPDATE trade_rooms SET status='done' WHERE id=? AND status='confirm'", (body.trade_id,))
        await db.commit()

    async def load_u(tg):
        async with get_db() as db:
            r = await (await db.execute("SELECT balance, inventory FROM users WHERE tg_id=?", (tg,))).fetchone()
        if not r:
            return None
        try:
            inv = json.loads(r[1] or "[]")
        except Exception:
            inv = []
        return {"tg_id": tg, "balance": int(r[0] or 0), "inventory": inv if isinstance(inv, list) else []}

    async def save_u(uu):
        async with get_db() as db:
            await db.execute(
                "UPDATE users SET balance=?, inventory=? WHERE tg_id=?",
                (int(uu["balance"]), json.dumps(uu["inventory"], ensure_ascii=False), uu["tg_id"]),
            )
            await db.commit()

    ua, ub = await load_u(a_id), await load_u(b_id)
    if not ua or not ub:
        raise HTTPException(500, "user missing")

    def remove_items(inv, need):
        left = list(inv)
        for need_it in need:
            nid = (need_it.get("id") or "") + "|" + (need_it.get("name") or "") + "|" + str(need_it.get("value") or 0)
            found = False
            for i, it in enumerate(left):
                iid = (it.get("id") or "") + "|" + (it.get("name") or "") + "|" + str(it.get("value") or 0)
                if iid == nid or (it.get("name") == need_it.get("name") and int(it.get("value") or 0) == int(need_it.get("value") or 0)):
                    left.pop(i)
                    found = True
                    break
            if not found:
                raise HTTPException(400, "Предмет уже не в инвентаре — обнови оффер")
        return left

    # validate balances & items still owned
    if ua["balance"] < a_stars or ub["balance"] < b_stars:
        async with get_db() as db:
            await db.execute("UPDATE trade_rooms SET status='active', a_confirm=0, b_confirm=0, a_ready=0, b_ready=0 WHERE id=?", (body.trade_id,))
            await db.commit()
        raise HTTPException(400, "Не хватает ⭐ у одной из сторон")
    try:
        ua["inventory"] = remove_items(ua["inventory"], a_items)
        ub["inventory"] = remove_items(ub["inventory"], b_items)
    except HTTPException:
        async with get_db() as db:
            await db.execute("UPDATE trade_rooms SET status='active', a_confirm=0, b_confirm=0, a_ready=0, b_ready=0 WHERE id=?", (body.trade_id,))
            await db.commit()
    ...
    # fee on stars received (3%)
    a_gets_stars = int(b_stars * (1 - fee_rate))
    b_gets_stars = int(a_stars * (1 - fee_rate))
    ua["balance"] = ua["balance"] - a_stars + a_gets_stars
    ub["balance"] = ub["balance"] - b_stars + b_gets_stars
    ua["inventory"] = list(ua["inventory"]) + list(b_items)
    ub["inventory"] = list(ub["inventory"]) + list(a_items)
    await save_u(ua)
    await save_u(ub)
    try:
        await add_history(a_id, "trade", "win", f"deal #{body.trade_id}", a_gets_stars)
        await add_history(b_id, "trade", "win", f"deal #{body.trade_id}", b_gets_stars)
    except Exception:
        pass
    me_bal = ua["balance"] if int(u["tg_id"]) == a_id else ub["balance"]
    me_inv = ua["inventory"] if int(u["tg_id"]) == a_id else ub["inventory"]
    return {"ok": True, "done": True, "balance": me_bal, "inventory": me_inv, "fee_pct": 3}

@app.get("/api/trade/messages")
async def trade_messages(trade_id: int = Query(...), authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        await _ensure_trade_tables(db)
        row = await (await db.execute("SELECT a_id,b_id FROM trade_rooms WHERE id=?", (trade_id,))).fetchone()
        if not row or int(u["tg_id"]) not in (int(row[0] or 0), int(row[1] or 0)):
            raise HTTPException(403, "Нет доступа")
        rows = await (await db.execute(
            "SELECT username, text, created_at, tg_id FROM trade_messages WHERE trade_id=? ORDER BY id DESC LIMIT 50",
            (trade_id,),
        )).fetchall()
    return {"messages": [{"user": r[0], "text": r[1], "ts": r[2], "tg_id": int(r[3] or 0)} for r in reversed(rows or [])]}

@app.post("/api/trade/message")
async def trade_message(body: TradeMsgBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    _trade_rate(u["tg_id"], limit=20)
    text = (body.text or "").strip()[:240]
    if not text:
        raise HTTPException(400, "Пусто")
    async with get_db() as db:
        await _ensure_trade_tables(db)
        row = await (await db.execute("SELECT a_id,b_id FROM trade_rooms WHERE id=?", (body.trade_id,))).fetchone()
        if not row or int(u["tg_id"]) not in (int(row[0] or 0), int(row[1] or 0)):
            raise HTTPException(403, "Нет доступа")
        await db.execute(
            "INSERT INTO trade_messages(trade_id,tg_id,username,text,created_at) VALUES(?,?,?,?,?)",
            (int(body.trade_id), u["tg_id"], u.get("username") or "Player", text, now_ts()),
        )
        await db.commit()
    return {"ok": True}


# -------------------- SKIP STONE (камень по воде) --------------------
SKIP_GAMES: Dict[str, dict] = {}
# difficulty: p_continue per bounce, max 15, house edge built-in
SKIP_DIFF = {
    "easy":   {"p": 0.72, "base": 1.12, "label": "Лёгкий"},
    "medium": {"p": 0.58, "base": 1.28, "label": "Средний"},
    "hard":   {"p": 0.45, "base": 1.48, "label": "Сложный"},
}

class SkipStartBody(BaseModel):
    bet: int
    difficulty: str = "medium"

class SkipGoBody(BaseModel):
    game_id: str

@app.post("/api/skip/start")
async def skip_start(body: SkipStartBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    bet = check_bet(int(body.bet))
    diff = (body.difficulty or "medium").lower()
    if diff not in SKIP_DIFF:
        raise HTTPException(400, "difficulty: easy|medium|hard")
    if u["balance"] < bet:
        raise HTTPException(400, "Недостаточно ⭐")
    lock = await user_lock(u["tg_id"])
    async with lock:
        ...
        gid = uuid.uuid4().hex
        SKIP_GAMES[gid] = {
            "tg_id": u["tg_id"], "bet": bet, "diff": diff,
            "bounces": 0, "mult": 1.0, "alive": True, "cashed": False, "ts": now_ts(),
        }
        await save_user(u)
    return {"game_id": gid, "balance": u["balance"], "mult": 1.0, "bounces": 0, "diff": diff}

@app.post("/api/skip/throw")
async def skip_throw(body: SkipGoBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    """Один бросок/отскок. Успех → mult растёт, провал → lose."""
    u = await current_user(authorization, x_telegram_init_data)
    g = SKIP_GAMES.get(body.game_id)
    if not g or g["tg_id"] != u["tg_id"]:
        raise HTTPException(400, "Нет игры")
    if not g["alive"] or g["cashed"]:
        raise HTTPException(400, "Игра окончена")
    cfg = SKIP_DIFF[g["diff"]]
    # probability slightly drops with each bounce
    p = cfg["p"] * (0.97 ** g["bounces"])
    p = max(0.18, min(0.85, p))
    ok = random.SystemRandom().random() < p
    if not ok:
        g["alive"] = False
        await add_history(u["tg_id"], "skip", "lose", f"{g['diff']} b{g['bounces']}", 0)
        return {"ok": False, "sunk": True, "bounces": g["bounces"], "mult": round(g["mult"], 2), "balance": u["balance"]}
    g["bounces"] += 1
    # mult growth
    g["mult"] = round(g["mult"] * cfg["base"], 4)
    if g["bounces"] >= 15:
        # auto max — force cashout path
        return {"ok": True, "sunk": False, "bounces": g["bounces"], "mult": round(g["mult"], 2), "max": True, "can_cashout": True}
    return {"ok": True, "sunk": False, "bounces": g["bounces"], "mult": round(g["mult"], 2), "can_cashout": True}

@app.post("/api/skip/cashout")
async def skip_cashout(body: SkipGoBody, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    g = SKIP_GAMES.get(body.game_id)
    if not g or g["tg_id"] != u["tg_id"]:
        raise HTTPException(400, "Нет игры")
    if g["cashed"] or not g["alive"]:
        raise HTTPException(400, "Нельзя")
    if g["bounces"] < 1:
        raise HTTPException(400, "Сначала отскок")
    g["cashed"] = True
    g["alive"] = False
    # house edge extra 2%
    win = clamp_win(int(g["bet"] * g["mult"] * (1 - max(HOUSE_EDGE, 0.02))))
    u["balance"] += win
    u["wins"] += 1
    await save_user(u)
    await add_history(u["tg_id"], "skip", "win", f"x{g['mult']:.2f} b{g['bounces']}", win)
    return {"win": win, "balance": u["balance"], "mult": round(g["mult"], 2), "bounces": g["bounces"]}
...
async def battle_list():
    async with get_db() as db:
        rows = await (await db.execute(
            "SELECT id, host_id, host_name, case_id, price, status, created_at FROM battles WHERE status='open' ORDER BY id DESC LIMIT 30"
        )).fetchall()
    return {"battles": [
        {"id": int(r[0]), "host_id": int(r[1] or 0), "host": r[2] or "Player", "case_id": r[3], "price": int(r[4] or 0), "status": r[5], "ts": r[6]}
        for r in (rows or [])
    ]}

@app.post("/api/battle/create")
async def battle_create(body: BattleCreate, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    c = CASES.get(body.case_id)
    if not c or int(c.get("price") or 0) <= 0:
        raise HTTPException(400, "Выбери платный кейс")
    price = int(c["price"])
    if u["balance"] < price:
        raise HTTPException(400, "Недостаточно ⭐")
    u["balance"] = int(u["balance"]) - price
    await save_user(u)
    async with get_db() as db:
        await db.execute(
            "INSERT INTO battles(host_id,host_name,guest_id,guest_name,case_id,price,host_drop,guest_drop,winner_id,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
            (u["tg_id"], u.get("username") or "Player", None, None, body.case_id, price, None, None, None, "open", now_ts()),
        )
        await db.commit()
        row = await (await db.execute("SELECT id FROM battles WHERE host_id=? ORDER BY id DESC LIMIT 1", (u["tg_id"],))).fetchone()
    return {"ok": True, "battle_id": int(row[0]) if row else 0, "balance": u["balance"], "price": price, "case": c.get("name")}

@app.post("/api/battle/join")
async def battle_join(body: BattleJoin, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT id, host_id, host_name, case_id, price, status FROM battles WHERE id=?", (body.battle_id,)
        )).fetchone()
        if not row:
            raise HTTPException(404, "battle")
        if (row[5] or "") != "open":
            raise HTTPException(400, "Комната закрыта")
        host_id = int(row[1])
        if host_id == int(u["tg_id"]):
            raise HTTPException(400, "Нельзя войти в свою комнату")
        price = int(row[4] or 0)
        case_id = row[3]
        c = CASES.get(case_id)
        if not c:
            raise HTTPException(400, "Кейс недоступен")
        if u["balance"] < price:
            raise HTTPException(400, "Недостаточно ⭐")
        # lock room
        await db.execute(
            "UPDATE battles SET status='running', guest_id=?, guest_name=? WHERE id=? AND status='open'",
            (u["tg_id"], u.get("username") or "Player", body.battle_id),
        )
        await db.commit()
    ...
    # rolls
    drop_h = roll_case_drop(case_id, c)
    drop_g = roll_case_drop(case_id, c)

    def drop_value(d):
        if d.get("kind") == "gift" and d.get("gift"):
            return int(d["gift"].get("value") or 0)
        return int(d.get("stars") or 0)

    def drop_public(d):
        if d.get("kind") == "gift" and d.get("gift"):
            g = d["gift"]
            return {"kind": "gift", "name": g.get("name"), "value": g.get("value"), "img": g.get("img"), "emoji": g.get("emoji") or "🎁"}
        return {"kind": "stars", "name": "⭐", "value": int(d.get("stars") or 0), "emoji": "⭐"}

    vh, vg = drop_value(drop_h), drop_value(drop_g)
    # tiny house edge: if close, slight bias random already; winner takes both
    if vh > vg:
        winner = host_id
    elif vg > vh:
        winner = int(u["tg_id"])
    else:
        winner = host_id if random.random() < 0.5 else int(u["tg_id"])

    # award both drops to winner
    async def give_drop(tg, d):
        async with get_db() as db:
            rowu = await (await db.execute("SELECT balance, inventory FROM users WHERE tg_id=?", (tg,))).fetchone()
            if not rowu:
                return
            bal = int(rowu[0] or 0)
            try:
                inv = json.loads(rowu[1] or "[]")
            except Exception:
                inv = []
            if d.get("kind") == "gift" and d.get("gift"):
                g = dict(d["gift"])
            ...
            await db.execute("UPDATE users SET balance=?, inventory=? WHERE tg_id=?", (bal, json.dumps(inv, ensure_ascii=False), tg))
            await db.commit()

    await give_drop(winner, drop_h)
    await give_drop(winner, drop_g)

    ph, pg = drop_public(drop_h), drop_public(drop_g)
    async with get_db() as db:
        await db.execute(
            "UPDATE battles SET status='done', host_drop=?, guest_drop=?, winner_id=? WHERE id=?",
            (json.dumps(ph, ensure_ascii=False), json.dumps(pg, ensure_ascii=False), winner, body.battle_id),
        )
        await db.commit()

    # refresh guest balance
    u2 = await current_user(authorization, x_telegram_init_data)
    return {
        "ok": True,
        "winner_id": winner,
        "you_win": winner == int(u["tg_id"]),
        "host_drop": ph,
        "guest_drop": pg,
        "balance": u2["balance"],
        "inventory": u2["inventory"],
    }

@app.post("/api/battle/cancel")
async def battle_cancel(body: BattleJoin, authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    u = await current_user(authorization, x_telegram_init_data)
    async with get_db() as db:
        row = await (await db.execute(
            "SELECT id, host_id, price, status FROM battles WHERE id=?", (body.battle_id,)
        )).fetchone()
        if not row:
            raise HTTPException(404, "battle")
        if int(row[1]) != int(u["tg_id"]):
            raise HTTPException(403, "Не твоя комната")
        if (row[3] or "") != "open":
            raise HTTPException(400, "Уже начат")
        await db.execute("UPDATE battles SET status='cancelled' WHERE id=? AND status='open'", (body.battle_id,))
        await db.commit()
    u["balance"] = int(u["balance"]) + int(row[2] or 0)
    await save_user(u)
    return {"ok": True, "balance": u["balance"]}
    ...
    while True:
        try:
            ...
            while m < CRASH["crash_point"]:
                # ~x2 за ~24 сек, дальше медленнее (как в нормальных crash)
                # growth ~0.38%/тик @ 150ms ≈ x2 за ~27с
                m = min(CRASH["crash_point"], m * 1.0038 + 0.00025)
                CRASH["multiplier"] = round(m, 2)
                tick += 1
                # emit каждый 2-й тик — меньше нагрузка на клиентов
                if tick % 2 == 0 or m >= CRASH["crash_point"]:
