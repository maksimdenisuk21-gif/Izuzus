...
<div class="row"><button onclick="loadWd()">Обновить выводы</button></div>
...
async def leaderboard():
    ...
    return {"by_balance": pack(by_bal), "by_wins": pack(by_wins), "by_cases": pack(by_cases), "by_new": pack(by_new)}
...
async def admin_give(
    ...
    async with get_db() as db:
        ...
        if not row:
            # создаём игрока, если ещё не заходил
            await db.execute(
                "INSERT INTO users(tg_id,username,balance,inventory,games,wins,deposited,free_case_at,friend_case_at,chance_bonus,ton_wallet,cases_opened,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (uid, f"user_{uid}", max(0, amt), "[]", 0, 0, 0, 0, 0, 0, "", 0, now_ts()),
            )
            await db.commit()
            new_bal = max(0, amt)
        else:
...
async def admin_users(
    ...
    async with get_db() as db:
        if (q or "").strip().isdigit():
            rows = await (await db.execute(
                "SELECT tg_id, username, balance, deposited, games, wins, cases_opened FROM users WHERE tg_id=? LIMIT 1",
                (int(q.strip()),),
            )).fetchall()
        elif (q or "").strip():
            like = "%" + q.strip().replace("%", "") + "%"
            rows = await (await db.execute(
                "SELECT tg_id, username, balance, deposited, games, wins, cases_opened FROM users WHERE username LIKE ? ORDER BY balance DESC LIMIT ?",
                (like, lim),
            )).fetchall()
            if not rows:
                rows = await (await db.execute(
            ...
            rows = await (await db.execute(
                "SELECT tg_id, username, balance, deposited, games, wins, cases_opened FROM users ORDER BY balance DESC LIMIT ?",
                (lim,),
            )).fetchall()
    return {
        "users": [
