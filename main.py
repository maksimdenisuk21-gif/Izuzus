...
CASES = {
    ...
    'star_case_1': {'name': '⭐ STAR CASE I', 'price': 55, 'category': 'stars', 'icon': '⭐', 'color': 'c-starter', 'star_drops': [15, 25, 35, 45, 55, 80, 120, 200], 'star_weights': [38, 30, 15, 8, 5, 2.5, 1, 0.5], 'desc': 'RTP ~72% · редко выше цены'},
    'star_case_2': {'name': '⭐ STAR CASE II', 'price': 110, 'category': 'stars', 'icon': '✨', 'color': 'c-starter', 'star_drops': [25, 45, 65, 90, 110, 170, 280, 450], 'star_weights': [38, 30, 15, 8, 5, 2.5, 1, 0.5], 'desc': 'RTP ~72%'},
    'star_case_3': {'name': '⭐ STAR CASE III', 'price': 280, 'category': 'stars', 'icon': '🌟', 'color': 'c-pepe', 'star_drops': [60, 100, 150, 220, 280, 430, 700, 1100], 'star_weights': [38, 30, 15, 8, 5, 2.5, 1, 0.5], 'desc': 'RTP ~72%'},
    'star_case_4': {'name': '⭐ STAR CASE IV', 'price': 550, 'category': 'stars', 'icon': '💫', 'color': 'c-tg', 'star_drops': [120, 200, 300, 430, 550, 850, 1400, 2500], 'star_weights': [38, 30, 15, 8, 5, 2.5, 1, 0.5], 'desc': 'RTP ~72%'},
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    if tv <= 0 or iv <= 0:
        return 0.01
    raw = (iv / tv) * 100.0
    # x2 ≈ 47% (50*0.94) → RTP ~94%, «почти 50/50», касса ~6%
    ch = max(0.01, min(80.0, raw * 0.94))
    ch = min(90.0, ch + min(3.0, float(bonus or 0)))
    return round(ch, 2)
...
def _slot_targets(price: int) -> list:
    """8 слотов относительно цены кейса (как в таблице экономики)."""
    p = max(1, int(price or 1))
    # 20-30%, 30-40%, 40-55%, 55-70%, 90-110%, 130-170%, 200-300%, 400%+
    bands = [
        (0.20, 0.30),
        (0.30, 0.40),
        (0.40, 0.55),
    ...
    out = []
    for lo, hi in bands:
        v = int(round(p * (lo + hi) / 2))
        out.append(max(1, v))
    return out


SLOT_WEIGHTS = [38, 30, 15, 8, 5, 2.5, 1, 0.5]


def _gift_near_value(target: int, prefer_cheap: bool = True):
    target = max(1, int(target))
    tol = max(25, int(target * 0.35))
    band = [g for g in GIFTS_FLAT if abs(int(g.get("value") or 0) - target) <= tol]
    if not band:
        band = sorted(GIFTS_FLAT, key=lambda g: abs(int(g.get("value") or 0) - target))[:12]
    ww = []
    for g in band:
        v = int(g.get("value") or 0)
        dist = abs(v - target) + 15
        cheap = 1.35 if (prefer_cheap and v <= target) else 1.0
        ww.append(cheap / dist)
    g = weighted_pick(band, ww) or band[0]
    return gift_public(g)


def roll_case_drop(case_id: str, c: dict) -> dict:
    """Единая экономика: 8 слотов [38,30,15,8,5,2.5,1,0.5]. Free — только мелкие ⭐."""
    price = int(c.get("price") or 0)

    # FREE DAILY — только 1–10⭐ (часто 1–3), без NFT
    if case_id == "free_daily" or (price <= 0 and c.get("category") == "free"):
        ...
        stars = int(weighted_pick(drops, weights) or 1)
    ...
    # STAR cases — только звёзды по таблице
    star_drops = c.get("star_drops") or []
    if star_drops and (c.get("category") == "stars" or (not c.get("force_names") and not c.get("rarities") and price > 0)):
        if c.get("category") == "stars" or not (c.get("force_names") or c.get("rarities")):
            sw = c.get("star_weights") or SLOT_WEIGHTS
            stars = int(weighted_pick(star_drops, sw) or star_drops[0])
            return {"kind": "stars", "stars": stars}

    # NFT / бренды / themed — 8 слотов value → NFT ≈ target
    if price > 0:
        targets = _slot_targets(price)
        # если в кейсе заданы свои value_drops — используем
        if c.get("value_drops") and len(c["value_drops"]) >= 8:
            targets = [int(x) for x in c["value_drops"][:8]]
        slot = weighted_pick(list(range(8)), SLOT_WEIGHTS) or 0
        target = targets[int(slot)]
        # force_names: предпочитаем их, если value близко
        ...
        for n in names:
            ...
            if g:
                pool.append(g)
        if pool:
            # среди force_names выбрать ближе к target (с bias дешевле)
            ww = []
            for g in pool:
                v = max(1, int(g.get("value") or 50))
                dist = abs(v - target) + 20
                ww.append((1.4 if v <= target else 1.0) / dist)
            g = weighted_pick(pool, ww) or pool[0]
            # если force name сильно дороже target на ранних слотах — иногда подменить каталогом
            gv = int(g.get("value") or 0)
            if slot <= 3 and gv > target * 1.8 and random.random() < 0.7:
                return {"kind": "gift", "gift": _gift_near_value(target)}
            return {"kind": "gift", "gift": gift_public(g)}
        return {"kind": "gift", "gift": _gift_near_value(target)}
...
async def _do_upgrade(body: UpgradeRequest, authorization, x_telegram_init_data):
    ...
    # каждый апгрейд независим (не зависит от прошлых)
    success = (random.SystemRandom().random() * 100.0) < chance
