...
# ========== CRASH ==========
CRASH_MIN_BET = 25; CRASH_MAX_BET = 5000; CRASH_BETTING_TIME = 6; CRASH_COOLDOWN = 3
CRASH_HOUSE_EDGE = 0.04; CRASH_SPEED = 0.08
...
REFERRAL_PERCENT = 7; MAX_WITHDRAW_AMOUNT = 50000; WITHDRAW_FEE = 0.05
WITHDRAW_COOLDOWN_HOURS = 24; MIN_DEPOSIT_FOR_REFERRAL = 50
...
crash_state = {
    "status": "waiting", "round_id": "", "crash_point": 1.0, "hash": "",
    "start_time": 0, "bets": {}, "history": [], "timer_ends": 0,
    "connected_users": set(), "current_multiplier": 1.0, "crashed": False
}
...
def generate_crash_point():
    ...
    if rounds_since_seed_change >= ROUNDS_BEFORE_SEED_CHANGE:
        ...
        SERVER_SEED_HASH = hashlib.sha256(SERVER_SEED.encode()).hexdigest()
        crash_nonce = 0; rounds_since_seed_change = 0
...
async def place_bet(sid, data):
    ...
    crash_state["bets"][key] = {
        "tg_id": tg_id, "bet": bet, "username": data.get('username', 'Игрок'),
        "round_id": crash_state["round_id"], "cashed_out": False, "cashed_at": 0, "sid": sid
    }
    
    await sio.emit('bet_placed', {
        'tg_id': tg_id, 'username': data.get('username', 'Игрок'),
        'amount': bet, 'balance': new_balance, 'round_id': crash_state["round_id"]
    }, to=sid)
...
async def cashout(sid, data):
    ...
    await sio.emit('cashout_success', {
        ...
        'profit': win_amount - bet_data["bet"], 'balance': new_balance,
...
def verify_telegram_data(authorization: str = Header(None)):
    ...
    try:
        ...
        if calculated_hash != hash_value: raise HTTPException(status_code=401, detail="Data validation failed")
...
async def create_promo(req: PromoCreateRequest, user: dict = Depends(verify_telegram_data)):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.execute("INSERT INTO promos (code, reward_type, case_type, stars, max_uses, created_by) VALUES (?,?,?,?,?,?)",
...
async def reward_top_players(user: dict = Depends(verify_telegram_data)):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        async with db.execute("SELECT tg_id, username, balance FROM users ORDER BY balance DESC LIMIT 5") as cursor:
...
async def open_case(req: OpenCaseRequest, user: dict = Depends(verify_telegram_data)):
    if req.case_type in STAR_CASE_PRICES:
        price = STAR_CASE_PRICES[req.case_type]; pool = STAR_CASE_DROPS[req.case_type]; weights = STAR_DROP_WEIGHTS
    elif req.case_type in NFT_CASE_PRICES:
        price = NFT_CASE_PRICES[req.case_type]; pool = NFT_CASE_DROPS[req.case_type]; weights = NFT_DROP_WEIGHTS
        ...
        for ct, drops in {**STAR_CASE_DROPS, **NFT_CASE_DROPS}.items():
            for did, (dname, dprice) in drops.items():
                if dprice == req.target_price:
                    new_item = {"id": did, "name": dname, "case": ct}; break
            if new_item: break
...
async def buy_stars(stars_amount: int, user: dict = Depends(verify_telegram_data)):
    ...
    async with httpx.AsyncClient() as client:
        res = await client.post(f"https://api.telegram.org/bot{BOT_TOKEN}/createInvoiceLink", json={
            "title": "Пополнение баланса", "description": f"Покупка {stars_amount} Stars",
            "payload": f"deposit_{tg_id}_{stars_amount}", "provider_token": "",
            "currency": "XTR", "prices": [{"label": "Stars", "amount": stars_amount}]
        })
        res_data = res.json()
        if res_data.get("ok"):
...
async def create_withdraw(amount: int, wallet: str, user: dict = Depends(verify_telegram_data)):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        fee = int(amount * WITHDRAW_FEE); payout = amount - fee
