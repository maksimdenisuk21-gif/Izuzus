...
HOUSE_EDGE = float(os.getenv("HOUSE_EDGE") or "0.035")  # RTP ≈ 96.5%, мягкая касса
MIN_BET = 50
...
def roll_crash_point() -> float:
    ...
    # ~3% краш 1.00x, фактор 0.97 → RTP ~96–97%
    if r < 0.03:
        return 1.0
    p = max(1.01, 0.97 / (1 - r))
...
SLOT_WEIGHTS = [22, 20, 18, 14, 12, 8, 4, 2]  # RTP ~96–97%: чаще около окупа
...
async def leaderboard():
    async with get_db() as db:
        by_bal = await (await db.execute(
            "SELECT username, COALESCE(balance,0), COALESCE(wins,0), COALESCE(cases_opened,0), tg_id FROM users ORDER BY balance DESC LIMIT 30"
        )).fetchall()
        by_wins = await (await db.execute(
            "SELECT username, COALESCE(balance,0), COALESCE(wins,0), COALESCE(cases_opened,0), tg_id FROM users ORDER BY wins DESC LIMIT 30"
        )).fetchall()
        by_cases = await (await db.execute(
            "SELECT username, COALESCE(balance,0), COALESCE(wins,0), COALESCE(cases_opened,0), tg_id FROM users ORDER BY cases_opened DESC LIMIT 30"
        )).fetchall()
        by_new = await (await db.execute(
            "SELECT username, COALESCE(balance,0), COALESCE(wins,0), COALESCE(cases_opened,0), tg_id FROM users ORDER BY created_at DESC LIMIT 30"
        )).fetchall()
    def pack(rows):
        return [{
            "name": (r[0] or "Player"),
    ...
    try:
        async with get_db() as db2:
            total_row = await (await db2.execute("SELECT COUNT(*) FROM users")).fetchone()
    except Exception:
        pass
    total = int(total_row[0] or 0) if total_row else 0
    return {"total_users": total, "by_balance": pack(by_bal), "by_wins": pack(by_wins), "by_cases": pack(by_cases), "by_new": pack(by_new)}
...
async def admin_backup(
    ...
    payload = {
        ...
        "users": [
            {"tg_id": int(r[0]), "username": r[1], "balance": int(r[2] or 0), "deposited": int(r[3] or 0),
             "games": int(r[4] or 0), "wins": int(r[5] or 0), "cases": int(r[6] or 0), "created_at": r[7]}
            for r in (users or [])
        ],
        "withdrawals": [
    ...
    for aid in _ADMIN_IDS:
        try:
            ...
            body = (
                ...
                f"--{boundary}\r\nContent-Disposition: form-data; name=\"caption\"\r\n\r\nBackup users={payload['users_count']}\r\n"
                f"--{boundary}\r\nContent-Disposition: form-data; name=\"document\"; filename=\"backup.json\"\r\n"
            ...
            with urllib.request.urlopen(req, timeout=30) as resp:
                sent.append({"admin": aid, "status": resp.status})
        except Exception as e:
            sent.append({"admin": aid, "error": str(e)})
