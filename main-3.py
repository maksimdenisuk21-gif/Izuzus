...
CASES = {
    ...
    'only_onyx': {'name': '🖤 ONYX BLACK', 'price': 1700, 'category': 'only_nft', 'icon': '🖤', 'color': 'c-durov', 'rarities': ['Epic', 'Legendary'], 'weights': [80, 20], 'stars_chance': 0.0, 'force_names': ['Candy Cane', 'Lol Pop', 'Evil Eye', 'Skull Flower', 'Top Hat', 'Electric Skull', 'Neko Helmet', 'Voodoo Doll'], 'desc': 'От 300⭐ · редко Neko/Voodoo'},
    'only_crystal': {'name': '💎 CRYSTAL VAULT', 'price': 2300, 'category': 'only_nft', 'icon': '💎', 'color': 'c-frag', 'rarities': ['Epic', 'Legendary', 'Mythic'], 'weights': [75, 20, 5], 'stars_chance': 0.0, 'force_names': ['Spy Agaric', 'Cookie Heart', 'Crystal Ball', 'Flying Broom', 'Diamond Ring', 'Ion Gem', 'Mini Oscar'], 'desc': 'От 300⭐ · редко Oscar'},
    'only_durov': {'name': '🧢 DUROV ONLY', 'price': 4000, 'category': 'only_nft', 'icon': '🧢', 'color': 'c-durov', 'rarities': ['Legendary', 'Mythic'], 'weights': [80, 20], 'stars_chance': 0.0, 'force_names': ['Top Hat', 'Crystal Ball', "Khabib's Papakha", 'Diamond Ring', 'Mini Oscar', 'Heroic Helmet', 'Precious Peach'], 'desc': 'Чаще среднее, микро Pepe/Cap'},
    ...
    if rarities:
        ...
        if rar_pool:
            ...
            if pool:
                ...
                pick_from = rar_pool
            ww = [1.0 / max(10, int(x.get("value") or 50)) ** 0.65 for x in pick_from]
            g = weighted_pick(pick_from, ww) or pick_from[0]
            return {"kind": "gift", "gift": gift_public(g)}
    if pool:
        # чаще дешёвые гифты → EV < цена кейса
        w = []
        for g in pool:
        ...
        return {"kind": "gift", "gift": gift_public(g)}
    # fallback stars
    return {"kind": "stars", "stars": random.randint(3, 25)}
