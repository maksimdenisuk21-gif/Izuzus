...
HOUSE_EDGE = float(os.getenv("HOUSE_EDGE") or "0.045")  # RTP ≈ 95.5% (+1% мягче)
MIN_BET = 50
...
CASES = {
    ...
    'star_case_1': {'name': '⭐ STAR CASE I', 'price': 55, 'category': 'stars', 'icon': '⭐', 'color': 'c-starter', 'star_drops': [15, 25, 35, 45, 55, 85, 125, 210], 'star_weights': [24, 21, 17, 13.5, 11.5, 7.5, 3.5, 2], 'desc': 'RTP ~72% · редко выше цены'},
    'star_case_2': {'name': '⭐ STAR CASE II', 'price': 110, 'category': 'stars', 'icon': '✨', 'color': 'c-starter', 'star_drops': [25, 45, 65, 90, 115, 175, 285, 460], 'star_weights': [24, 21, 17, 13.5, 11.5, 7.5, 3.5, 2], 'desc': 'RTP ~72%'},
    'star_case_3': {'name': '⭐ STAR CASE III', 'price': 280, 'category': 'stars', 'icon': '🌟', 'color': 'c-pepe', 'star_drops': [60, 100, 150, 220, 285, 430, 720, 1150], 'star_weights': [24, 21, 17, 13.5, 11.5, 7.5, 3.5, 2], 'desc': 'RTP ~72%'},
    'star_case_4': {'name': '⭐ STAR CASE IV', 'price': 550, 'category': 'stars', 'icon': '💫', 'color': 'c-tg', 'star_drops': [120, 200, 300, 430, 560, 860, 1420, 2550], 'star_weights': [24, 21, 17, 13.5, 11.5, 7.5, 3.5, 2], 'desc': 'RTP ~72%'},
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    raw = (iv / tv) * 100.0
    # x2 ≈ 47.75% → RTP ~95.5%
    ch = max(0.01, min(77.0, raw * 0.955))
    ch = min(87.0, ch + min(2.0, float(bonus or 0)))
    return round(ch, 2)
...
def mines_mult(mines: int, opened: int) -> float:
    ...
    for i in range(opened):
        ...
        if safe <= 0:
            break
        mult *= remaining / safe
    return max(1.0, mult * (1 - max(HOUSE_EDGE, 0.04)))  # мины ~ edge 4–5.5%
...
SLOT_WEIGHTS = [24, 21, 17, 13.5, 11.5, 7.5, 3.5, 2]  # RTP ~95.5% (+1%)
...
async def leaderboard():
    """Топ: все игроки (до 2000), без падений на NULL/типах."""
    ...
    try:
        async with get_db() as db:
            ...
            all_rows = await (await db.execute(
                "SELECT username, balance, wins, cases_opened, tg_id, created_at FROM users LIMIT 2000"
            )).fetchall()
        rows = []
        for r in (all_rows or []):
            try:
                rows.append((
                    ...
                    int(r[3] or 0),
                    int(r[4] or 0),
                    int(r[5] or 0) if r[5] is not None else 0,
                ))
            except Exception:
                continue
        def pack(sorted_rows):
            return [
                {"name": r[0], "balance": r[1], "wins": r[2], "cases": r[3], "tg_id": r[4]}
                for r in sorted_rows
            ]
        by_bal = pack(sorted(rows, key=lambda x: x[1], reverse=True))
        by_wins = pack(sorted(rows, key=lambda x: x[2], reverse=True))
        by_cases = pack(sorted(rows, key=lambda x: x[3], reverse=True))
        by_new = pack(sorted(rows, key=lambda x: x[5], reverse=True))
        return {
            "total_users": total,
            "by_balance": by_bal,
            "by_wins": by_wins,
...
async def admin_withdrawals(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"), x_admin_key: Optional[str] = Header(None, alias="X-Admin-Key")):
    ...
    try:
        async with get_db() as db:
            rows = await (await db.execute(
                "SELECT id, tg_id, amount, method, dest, note, status, created_at FROM withdrawals ORDER BY id DESC LIMIT 100"
            )).fetchall()
        for r in (rows or []):
            try:
                items.append({
        ...
        return {"items": [], "withdrawals": [], "list": [], "error": str(e)}
