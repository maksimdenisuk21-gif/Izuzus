...
async def craft_items(
    ...
    u = await current_user(authorization, x_telegram_init_data)
    ids = [int(x) for x in (body.item_ids or []) if str(x).isdigit() or isinstance(x, int)]
    # also accept string ids
    if not ids:
        raw = body.item_ids or []
        ids = []
        for x in raw:
            try:
                ids.append(int(x))
    ...
    async with get_db() as db:
        ...
        for it in inv:
            ...
            if iid in idset and len(selected) < len(ids):
                selected.append(it)
                idset.discard(iid)
            else:
                remaining.append(it)
        if len(selected) < 3:
            raise HTTPException(400, "Не найдены предметы в инвентаре")

        total = sum(max(1, int(it.get("value") or 0)) for it in selected)
        # RTP ~88%
        target = max(1, int(total * 0.88))
        # pick from known gifts near target
        pool = list(GIFTS_FLAT) if GIFTS_FLAT else []
        if not pool:
            # fallback synthetic
            pool = [
                {"name": "Подарок", "value": max(1, target), "rarity": "Common", "emoji": "🎁", "sn": "gift"},
            ]
        # sort by closeness to target
        scored = sorted(pool, key=lambda g: abs(int(g.get("value") or 0) - target))
        # weighted among top 12
        top = scored[:12] or scored
        weights = []
        for g in top:
