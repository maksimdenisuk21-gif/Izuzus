...
async def my_withdrawals(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data")):
    ...
    items = [
        {"id": int(r[0]), "amount": int(r[1] or 0), "method": r[2] or "", "dest": r[3] or "", "status": r[4] or "", "ts": r[5]}
        for r in (rows or [])
    ]
    ...
    async with lock:
        async with get_db() as db:
            ...
            try:
                ...
                print("[withdraw insert]", e)
                raise HTTPException(500, f"Не удалось создать заявку: {e}")
            new_bal = bal - amount
    ...
    try:
        async with get_db() as db:
            try:
                ...
                print("[admin/withdrawals] cols", e1)
                rows = await (await db.execute(
                    "SELECT id, tg_id, amount, method, dest, status, created_at FROM withdrawals ORDER BY id DESC LIMIT 200"
                )).fetchall()
                # normalize to 8-tuple shape
                rows = [(r[0], r[1], r[2], r[3], r[4], "", r[5], r[6] if len(r) > 6 else None) for r in (rows or [])]
        for r in (rows or []):
            try:
                items.append({
                    "id": int(r[0]),
