...
CASES = {
    ...
    'only_onyx': {'name': '🖤 ONYX BLACK', 'price': 1700, 'category': 'only_nft', 'icon': '🖤', 'color': 'c-durov', 'rarities': ['Epic', 'Legendary'], 'weights': [80, 20], 'stars_chance': 0.40, 'force_names': ['Candy Cane', 'Lol Pop', 'Evil Eye', 'Skull Flower', 'Top Hat', 'Electric Skull', 'Neko Helmet', 'Voodoo Doll'], 'desc': 'От 300⭐ · редко Neko/Voodoo'},
    'only_crystal': {'name': '💎 CRYSTAL VAULT', 'price': 2300, 'category': 'only_nft', 'icon': '💎', 'color': 'c-frag', 'rarities': ['Epic', 'Legendary', 'Mythic'], 'weights': [75, 20, 5], 'stars_chance': 0.40, 'force_names': ['Spy Agaric', 'Cookie Heart', 'Crystal Ball', 'Flying Broom', 'Diamond Ring', 'Ion Gem', 'Mini Oscar'], 'desc': 'От 300⭐ · редко Oscar'},
    'only_durov': {'name': '🧢 DUROV ONLY', 'price': 4000, 'category': 'only_nft', 'icon': '🧢', 'color': 'c-durov', 'rarities': ['Legendary', 'Mythic'], 'weights': [80, 20], 'stars_chance': 0.40, 'force_names': ['Top Hat', 'Crystal Ball', "Khabib's Papakha", 'Diamond Ring', 'Mini Oscar', 'Heroic Helmet', 'Precious Peach'], 'desc': 'Чаще среднее, микро Pepe/Cap'},
...
def _cheap_fallback_stars(price: int) -> dict:
    """Звёзды ниже цены, но не копейки."""
    price = max(0, int(price or 0))
    if price <= 0:
        return {"kind": "stars", "stars": random.randint(5, 25)}
    r = random.random()
    if r < 0.55:
        # часто ~35–55% цены
        lo = max(8, int(price * 0.30))
        hi = max(lo + 5, int(price * 0.55))
    elif r < 0.90:
        # иногда ~55–80%
        lo = max(10, int(price * 0.50))
        hi = max(lo + 5, int(price * 0.80))
    else:
        # редко почти окуп 80–100%
        lo = max(15, int(price * 0.75))
        hi = max(lo + 5, int(price * 1.00))
...
def _pick_gift_for_case(pool: list, price: int) -> dict:
    ...
    # сильно дороже цены — чаще режем, но не 97%
    if val > price * 2.0:
...
def roll_case_drop(case_id: str, c: dict) -> dict:
    ...
    # умеренно чаще звёзды на дорогих кейсах
    if price >= 800:
        stars_chance = max(stars_chance, 0.35)
    if price >= 1500:
        stars_chance = max(stars_chance, 0.42)
    if stars_chance > 0 and random.random() < stars_chance:
        lo = int(c.get("min_stars") or 1)
        hi = int(c.get("max_stars") or 20)
        if price > 0:
            # звёзды в диапазоне ~25–70% цены
            lo = max(lo, int(price * 0.20))
            hi = min(max(hi, lo + 5), max(lo + 10, int(price * 0.70)))
        if c.get("stars_bias_low"):
            stars = int(min(hi, max(lo, random.paretovariate(1.5) + lo - 1)))
        else:
