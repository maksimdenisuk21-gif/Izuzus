...
CASES = {
    ...
    # RTP ~96%: weights [25,25,20,12,8,5,3,2] × multipliers [0.5,0.7,0.9,1.0,1.2,1.8,3.0,5.0]
    'star_case_1': {'name': '⭐ STAR CASE I', 'price': 55, 'category': 'stars', 'icon': '⭐', 'color': 'c-starter', 'star_drops': [28, 38, 50, 55, 66, 99, 165, 275], 'star_weights': [25, 25, 20, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 28–275⭐'},
    'star_case_2': {'name': '⭐ STAR CASE II', 'price': 110, 'category': 'stars', 'icon': '✨', 'color': 'c-starter', 'star_drops': [55, 77, 99, 110, 132, 198, 330, 550], 'star_weights': [25, 25, 20, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 55–550⭐'},
    'star_case_3': {'name': '⭐ STAR CASE III', 'price': 280, 'category': 'stars', 'icon': '🌟', 'color': 'c-pepe', 'star_drops': [140, 196, 252, 280, 336, 504, 840, 1400], 'star_weights': [25, 25, 20, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 140–1400⭐'},
    'star_case_4': {'name': '⭐ STAR CASE IV', 'price': 550, 'category': 'stars', 'icon': '💫', 'color': 'c-tg', 'star_drops': [275, 385, 495, 550, 660, 990, 1650, 2750], 'star_weights': [25, 25, 20, 12, 8, 5, 3, 2], 'desc': 'RTP ~96% · 275–2750⭐'},
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    if tv <= 0 or iv <= 0:
        return 0.01
    raw = (iv / tv) * 100.0
    # x2 ≈ 46% (raw=50 * 0.92) — мягкий плюс кассе, слив не резкий
    ch = max(0.01, min(78.0, raw * 0.92))
    ch = min(92.0, ch + float(bonus or 0))
    return round(ch, 2)

def roll_crash_point() -> float:
    r = random.random()
    # ~7% краш 1.00x, фактор 0.92 → edge ~8–10%, слив плавнее
    if r < 0.07:
        return 1.0
    p = max(1.01, 0.92 / (1 - r))
