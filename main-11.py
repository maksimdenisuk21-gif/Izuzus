...
HOUSE_EDGE = float(os.getenv("HOUSE_EDGE") or "0.10")  # RTP ≈ 90%
MIN_BET = 50
SHOP_MARKUP = 1.35
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    if tv <= 0 or iv <= 0:
        return 0.01
    raw = (iv / tv) * 100.0
    # x2 ≈ 45% (50 * 0.90) → EV ≈ -10%, каждый спин независим
    ch = max(0.01, min(75.0, raw * 0.90))
    # bonus только мелкий (админ), не разгоняет серию
    ch = min(85.0, ch + min(5.0, float(bonus or 0)))
    return round(ch, 2)

def roll_crash_point() -> float:
    r = random.SystemRandom().random()
    # ~8% краш 1.00x, фактор 0.90 → RTP ~90%
    if r < 0.08:
        return 1.0
    p = max(1.01, 0.90 / (1 - r))
...
def mines_mult(mines: int, opened: int) -> float:
    ...
    for i in range(opened):
        ...
        if safe <= 0:
            break
        mult *= remaining / safe
    return max(1.0, mult * (1 - max(HOUSE_EDGE, 0.10)))  # мины: RTP ~90%
...
def _pick_gift_for_case(pool: list, price: int) -> dict:
    ...
    # лёгкий дом: режем только очень жир
    if val > price * 3.0:
