...
HOUSE_EDGE = float(os.getenv("HOUSE_EDGE") or "0.055")  # RTP ≈ 94.5% (−2%)
MIN_BET = 50
...
CASES = {
    ...
    'star_case_1': {'name': '⭐ STAR CASE I', 'price': 55, 'category': 'stars', 'icon': '⭐', 'color': 'c-starter', 'star_drops': [15, 25, 35, 45, 55, 85, 125, 210], 'star_weights': [25, 22, 17, 13, 11, 7, 3.5, 1.5], 'desc': 'RTP ~72% · редко выше цены'},
    'star_case_2': {'name': '⭐ STAR CASE II', 'price': 110, 'category': 'stars', 'icon': '✨', 'color': 'c-starter', 'star_drops': [25, 45, 65, 90, 115, 175, 285, 460], 'star_weights': [25, 22, 17, 13, 11, 7, 3.5, 1.5], 'desc': 'RTP ~72%'},
    'star_case_3': {'name': '⭐ STAR CASE III', 'price': 280, 'category': 'stars', 'icon': '🌟', 'color': 'c-pepe', 'star_drops': [60, 100, 150, 220, 285, 430, 720, 1150], 'star_weights': [25, 22, 17, 13, 11, 7, 3.5, 1.5], 'desc': 'RTP ~72%'},
    'star_case_4': {'name': '⭐ STAR CASE IV', 'price': 550, 'category': 'stars', 'icon': '💫', 'color': 'c-tg', 'star_drops': [120, 200, 300, 430, 560, 860, 1420, 2550], 'star_weights': [25, 22, 17, 13, 11, 7, 3.5, 1.5], 'desc': 'RTP ~72%'},
...
def calc_upgrade_chance(iv: float, tv: float, bonus: float = 0.0) -> float:
    ...
    raw = (iv / tv) * 100.0
    # x2 ≈ 47.25% → RTP ~94.5% (−2%)
    ch = max(0.01, min(76.0, raw * 0.945))
    ch = min(86.0, ch + min(2.0, float(bonus or 0)))
    return round(ch, 2)
...
SLOT_WEIGHTS = [25, 22, 17, 13, 11, 7, 3.5, 1.5]  # RTP ~94–95% (−2%)
...
async def leaderboard():
    """Все игроки из БД (без лимита 13/30)."""
    async with get_db() as db:
        total_row = await (await db.execute("SELECT COUNT(*) FROM users")).fetchone()
        total = int(total_row[0] or 0) if total_row else 0
        # все пользователи — сортировки на сервере
        all_rows = await (await db.execute(
            "SELECT username, COALESCE(balance,0), COALESCE(wins,0), COALESCE(cases_opened,0), tg_id, created_at FROM users"
        )).fetchall()
    rows = list(all_rows or [])
    def pack(sorted_rows):
        return [{
            "name": (r[0] or "Player"),
            "balance": int(r[1] or 0),
            "wins": int(r[2] or 0),
            "cases": int(r[3] or 0),
            "tg_id": int(r[4] or 0),
        } for r in sorted_rows]
    by_bal = pack(sorted(rows, key=lambda r: int(r[1] or 0), reverse=True))
    by_wins = pack(sorted(rows, key=lambda r: int(r[2] or 0), reverse=True))
    by_cases = pack(sorted(rows, key=lambda r: int(r[3] or 0), reverse=True))
    by_new = pack(sorted(rows, key=lambda r: int(r[5] or 0) if len(r) > 5 else 0, reverse=True))
    return {
        "total_users": total,
        "by_balance": by_bal,
        "by_wins": by_wins,
...
async def admin_withdrawals(authorization: Optional[str] = Header(None), x_telegram_init_data: Optional[str] = Header(None, alias="X-Telegram-Init-Data"), x_admin_key: Optional[str] = Header(None, alias="X-Admin-Key")):
    ...
    require_admin(me, x_admin_key)
    async with get_db() as db:
        rows = await (await db.execute("SELECT id, tg_id, amount, method, dest, note, status, created_at FROM withdrawals ORDER BY CASE WHEN status='pending' THEN 0 ELSE 1 END, id DESC LIMIT 80")).fetchall()
    items = [
